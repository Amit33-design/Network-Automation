...
// ─── IOS-XR PE Router Config (G-22 / G-39 — SP/MPLS) ─────────────────────────

function iosxrPEConfig(dev, state) {
  var unit  = dev.unit || 1;
  var lo0   = '10.0.0.' + unit;
  var lo1   = '10.0.1.' + unit;
  var asn   = state.bgp_asn || 65001;
  var rrIp  = '10.0.0.1';
  var nets  = (state.devices || []).filter(function(d) { return d.subLayer === 'p-router'; });
  var ifaces = [];
  for (var i = 0; i < Math.min(nets.length + 2, 4); i++) {
    ifaces.push({ name: 'GigabitEthernet0/0/0/' + i, ip: '10.1.' + unit + '.' + (i * 2) + '/31', desc: 'P-LINK-' + (i + 1) });
  }
  var hostname = dev.hostname || ('PE-' + unit);
  var vrfName  = 'CUST-A';
  var rd       = unit + ':100';

  var lines = [
    '!! IOS-XR PE Router — ' + hostname + ' (auto-generated by NetDesign AI)',
    'hostname ' + hostname,
    '!',
    'interface Loopback0',
    ' ipv4 address ' + lo0 + ' 255.255.255.255',
    '!',
    'interface Loopback1',
    ' ipv4 address ' + lo1 + ' 255.255.255.255',
    '  description SR Node SID',
    '!'
  ];

  ifaces.forEach(function(iface) {
    lines.push('interface ' + iface.name);
    lines.push(' description ' + iface.desc);
    lines.push(' ipv4 address ' + iface.ip);
    lines.push(' bfd minimum-interval 300');
    lines.push(' bfd multiplier 3');
    lines.push(' carrier-delay msec 0');
    lines.push(' load-interval 30');
    lines.push(' no shutdown');
    lines.push('!');
  });

  lines.push(
    '!! IS-IS underlay with Segment Routing',
    'router isis CORE',
    ' is-type level-2-only',
    ' net 49.0001.' + _hexPad(unit, 4) + '.0000.00',
    ' address-family ipv4 unicast',
    '  metric-style wide',
  ...
  ifaces.forEach(function(iface) {
    lines.push('router isis CORE interface ' + iface.name);
    lines.push(' circuit-type level-2-only');
    lines.push(' point-to-point');
    lines.push(' hello-interval 3');
    lines.push(' hold-count 3');
  ...
  if (_hasFeat(state, 'bfd')) {
    lines.push('bfd');
    lines.push(' echo disable');
    lines.push('!');
  }
...
function _hexPad(n, len) {
  var h = n.toString(16);
  while (h.length < len) h = '0' + h;
  return h;
}

// ─── IOS-XR P Router Config (G-22 / G-39) ─────────────────────────────────────

function iosxrPConfig(dev, state) {
  var unit     = dev.unit || 1;
  var lo0      = '10.0.2.' + unit;
  var asn      = state.bgp_asn || 65001;
  var hostname = dev.hostname || ('P-' + unit);

  var ifaces = [];
  var peDevs = (state.devices || []).filter(function(d) { return d.subLayer === 'pe-router'; });
  for (var i = 0; i < Math.min(peDevs.length + 2, 6); i++) {
    ifaces.push({ name: 'GigabitEthernet0/0/0/' + i, ip: '10.2.' + unit + '.' + (i * 2) + '/31', desc: 'PE-LINK-' + (i + 1) });
  }

  var lines = [
    '!! IOS-XR P Router — ' + hostname + ' (auto-generated by NetDesign AI)',
    'hostname ' + hostname,
    '!',
    'interface Loopback0',
    ' ipv4 address ' + lo0 + ' 255.255.255.255',
  ...
  lines.push(
    'router isis CORE',
    ' is-type level-2-only',
    ' net 49.0001.' + _hexPad(100 + unit, 4) + '.0000.00',
    ' address-family ipv4 unicast',
    '  metric-style wide',
    '  segment-routing mpls',
    '  mpls traffic-eng level-2-only',
...
// ─── Private 5G / O-RAN Config (G-40) ────────────────────────────────────────

function oranFronthaulConfig(dev, state) {
  var unit     = dev.unit || 1;
  var hostname = dev.hostname || ('FH-SW-' + unit);
  var mgmtIp   = '192.168.' + unit + '.10/24';
  var ptpGmIp  = state.ptp_gm_ip || '10.0.100.1';
  var vlan     = 100 + unit;

  var lines = [
    '! O-RAN Fronthaul Switch — ' + hostname + ' (NX-OS, PTP/eCPRI)',
    '! Generated by NetDesign AI',
    'hostname ' + hostname,
    '!',
    'feature lldp',
    ...
    'feature lacp',
    '!',
    'ptp source ip ' + mgmtIp.split('/')[0],
    'ptp mode boundary',
    'ptp priority1 128',
    'ptp priority2 ' + unit,
    'ptp domain 24',
    '!',
    '! SyncE for eCPRI fronthaul timing',
    'synce source Ethernet1/1',
    'synce quality-level QL-PRC',
    '!',
    'vlan ' + vlan,
    ...
    '  description UPLINK_MIDHAUL',
    '  no switchport',
    '  ip address 10.10.' + unit + '.0/31',
    '  ptp',
    '  spanning-tree port type edge',
    '  no shutdown',
    '!',
    '! eCPRI / Radio ports (Ethernet1/2 onward)',
    'interface Ethernet1/2',
    '  description ECPRI_RU_PORT',
    '  switchport',
    ...
    '  no shutdown',
    '!',
    '! QoS for eCPRI (latency-sensitive C-plane)',
    'class-map type qos match-all ECPRI-CTRL',
    '  match dscp cs7',
    'class-map type qos match-all ECPRI-USER',
    '  match dscp ef',
    '!',
    'policy-map type qos ECPRI-QOS',
    '  class ECPRI-CTRL',
    '    priority level 1',
    '  class ECPRI-USER',
    ...
    '!',
    'interface Ethernet1/2',
    '  service-policy type qos input ECPRI-QOS',
    '!'
  ];
...
function oranMidhaulConfig(dev, state) {
  var unit     = dev.unit || 1;
  var hostname = dev.hostname || ('MH-RTR-' + unit);

  var lines = [
    '! O-RAN Midhaul Router — ' + hostname + ' (IOS-XE)',
    '! Generated by NetDesign AI',
    'hostname ' + hostname,
    '!',
    'ptp mode boundary delay-req',
    'ptp clock-port UPSTREAM slave',
    ' ptp destination 10.0.100.1',
    '!',
    'ptp clock-port DOWNSTREAM master',
    ' ptp destination 10.10.' + unit + '.0',
    '!',
    '! Sync-E',
    'network-clock synchronization ssm option 1',
    'network-clock input-source 1 GigabitEthernet0/0/1',
    '!',
    'interface GigabitEthernet0/0/0',
    ' description BACKHAUL_UPLINK',
    ' ip address 10.20.' + unit + '.0 255.255.255.254',
    ' mpls ip',
    ' no shutdown',
    '!',
    'interface GigabitEthernet0/0/1',
    ' description FRONTHAUL_DOWNLINK',
    ' ip address 10.10.' + unit + '.1 255.255.255.254',
    ' no shutdown',
    '!',
    'router ospf 1',
    ' router-id 10.0.3.' + unit,
    ' network 0.0.0.0 255.255.255.255 area 0',
    '!',
    '! QoS — 5G midhaul priority classes',
    'class-map match-any 5G-CTRL',
    ' match dscp cs7 cs6',
    'class-map match-any 5G-USER',
    ' match dscp ef af41',
    '!',
    'policy-map 5G-MIDHAUL-QOS',
    ' class 5G-CTRL',
    '  priority percent 20',
    ' class 5G-USER',
...
// ─── Storage Networking Config (G-41) ─────────────────────────────────────────

function storageFabricConfig(dev, state) {
  var unit     = dev.unit || 1;
  var hostname = dev.hostname || ('MDS-' + unit);
  var vsanId   = 100 + unit;

  var lines = [
    '! Cisco MDS 9396T — ' + hostname + ' (NX-OS SAN)',
    '! NVMe/FC + FCoE Storage Fabric — Generated by NetDesign AI',
    'hostname ' + hostname,
    '!',
    'feature npiv',
    ...
    '!',
    'vsan database',
    '  vsan ' + vsanId + ' name PROD-SAN',
    '!',
    'zone smart-zoning enable vsan ' + vsanId,
    '!',
    '! NVMe/FC device-alias (add storage targets here)',
    'device-alias database',
    '  device-alias name NVME-ARRAY-1 pwwn 20:00:00:25:b5:01:00:01',
    '  device-alias name HOST-1-HBA1  pwwn 21:00:00:24:ff:01:00:01',
    'device-alias commit',
    '!',
    '! NVMe/FC zone',
    'zone name NVME-ZONE-1 vsan ' + vsanId,
    '  member device-alias HOST-1-HBA1   initiator',
    '  member device-alias NVME-ARRAY-1  target',
    '!',
    'zoneset name PROD-ZONESET vsan ' + vsanId,
    '  member NVME-ZONE-1',
    'zoneset activate name PROD-ZONESET vsan ' + vsanId,
    '!',
    '! ISL to peer MDS (PortChannel)',
    'interface port-channel 10',
    '  description ISL_TO_' + hostname + '_PEER',
    '  switchport mode E',
    '  channel mode active',
    '  no shutdown',
    '!',
    'interface fc1/1',
    '  description NVMe-Array-Port1',
    ...
    '  no shutdown',
    '!',
    '! FCIP gateway for WAN extension',
    'fcip enable',
    'fcip profile 1',
    '  ip address 10.0.50.' + unit,
    '!'
  ];
...
function storageLeafConfig(dev, state) {
  var unit     = dev.unit || 1;
  var hostname = dev.hostname || ('STOR-LEAF-' + unit);
  var lo0      = '10.0.60.' + unit;

  var lines = [
    '! Nexus 93600CD-GX — ' + hostname + ' (NVMe-oF / RoCEv2 leaf)',
    '! Generated by NetDesign AI',
    'hostname ' + hostname,
    '!',
    'feature bgp',
    ...
    'feature interface-vlan',
    '!',
    '! PFC/ECN for RoCEv2 / NVMe-oF (lossless fabric)',
    'system qos',
    '  service-policy type queuing input RDMA-IN-POLICY',
    '  service-policy type queuing output RDMA-OUT-POLICY',
    '!',
    'class-map type qos match-all RDMA-CLASS',
    '  match cos 3',
    'class-map type queuing RDMA-Q',
    '  match qos-group 3',
    '!',
    ...
    '    set qos-group 3',
    '!',
    'policy-map type queuing RDMA-IN-POLICY',
    '  class type queuing RDMA-Q',
    '    bandwidth remaining percent 50',
    '    pause pfc-cos 3',
    '!',
    'policy-map type queuing RDMA-OUT-POLICY',
    '  class type queuing RDMA-Q',
    '    bandwidth remaining percent 50',
    '    pause pfc-cos 3',
    '!',
    '! ECN for NVMe-oF congestion signaling',
    'hardware qos ecn minimum-absolute 150 maximum-absolute 1500',
    '!',
    'interface loopback0',
    '  description ROUTER-ID',
    '  ip address ' + lo0 + '/32',
    '!',
    '! iSCSI / NVMe-oF access ports',
    'interface Ethernet1/1',
    '  description STORAGE-HOST-1',
    '  switchport',
    '  switchport mode access',
    ...
    '  no shutdown',
    '!',
    '! Uplink to storage spine/core',
    'interface Ethernet1/49',
    '  description UPLINK-SPINE-1',
    '  no switchport',
    '  ip address 10.1.60.' + (unit * 2 - 1) + '/31',
    '  no shutdown',
    '!'
  ];
...
// ─── SD-WAN Full Architecture Config (G-42) ───────────────────────────────────

function sdwanControllerConfig(dev, state) {
  var unit     = dev.unit || 1;
  var hostname = dev.hostname || ('VSMART-' + unit);
  var systemIp = '10.0.200.' + unit;
  var siteId   = state.sdwan_site_id || unit;
  var orgName  = (state.org && state.org.name) || 'NetDesignOrg';

  var lines = [
    '!! Catalyst SD-WAN vSmart Controller — ' + hostname,
    '!! Generated by NetDesign AI',
    '!!',
    'system',
    ' host-name    ' + hostname,
    ...
    ' advertise static',
    '!',
    '!! Centralized Control Policy — App-aware routing',
    'policy',
    ' lists',
    '  site-list BRANCH-SITES',
    '   site-id 101-200',
    '  !',
    '  app-list REALTIME-APPS',
    '   app webex-media',
    '   app ms-teams-media',
    '   app zoom',
    '  !',
    ' !',
    ' control-policy APP-AWARE-ROUTING',
    '  sequence 10',
    '   match route',
    '    site-list BRANCH-SITES',
    '   !',
    ...
    ' !',
    '!',
    '!! Apply policy to all WAN edges',
    'apply-policy',
    ' site-list BRANCH-SITES',
    '  control-policy APP-AWARE-ROUTING out',
    '!'
  ];
...
function sdwanOrchConfig(dev, state) {
  var unit     = dev.unit || 1;
  var hostname = dev.hostname || ('VBOND-' + unit);
  var systemIp = '10.0.201.' + unit;
  var orgName  = (state.org && state.org.name) || 'NetDesignOrg';

  var lines = [
    '!! Catalyst SD-WAN vBond Orchestrator — ' + hostname,
    '!! Generated by NetDesign AI',
    '!!',
    'system',
    ' host-name    ' + hostname,
    ...
    ' site-id      1',
    ' organization-name ' + orgName,
    ' vbond ' + systemIp + ' local',
    '!',
    'vpn 0',
    ' interface ge0/0',
    '  ip address 10.0.201.' + unit + '/24',
    '  no tunnel-interface',
    '  no shutdown',
    ' !',
    '!',
    '!! vBond handles NAT traversal for WAN Edge onboarding.',
    '!! All vEdge/cEdge devices bootstrap to this address.',
    '!! Ensure DNS: ' + orgName.toLowerCase().replace(/\s+/g, '-') + '.vbond.sdwan.cisco.com → ' + systemIp
  ];
...
var VENDOR_GEN = {
  'Cisco': {
    ...
    'leaf':                nxosLeafConfig,
    'pe-router':           iosxrPEConfig,
    'p-router':            iosxrPConfig,
    'fronthaul':           oranFronthaulConfig,
    'midhaul':             oranMidhaulConfig,
    'storage-fabric':      storageFabricConfig,
    'storage-leaf':        storageLeafConfig,
    'sdwan-controller':    sdwanControllerConfig,
    'sdwan-orchestrator':  sdwanOrchConfig,
