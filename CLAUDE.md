...
| R3 (backend) | Port vendor profiles + `<CHANGE-ME>` Day-0 to `backend/ztp` — replace the hardcoded `ChangeMe!`/`NetDesignZTP1!` creds + hardcoded NTP/syslog IPs in the day0.j2 templates, add the 7 missing platforms (iosxr/srl/cumulus/dellos10/fortios/arubaoscx/exos/panos), and emit option-60 vendor classes in `dhcp_gen.py` | [ ] | mirrors `lib/ztp.ts`; backend pytest |
