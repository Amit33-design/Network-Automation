...
| AE3 | **The API's generation limits are discoverable — and the guard that enforces them now actually fires** — user decision (2026-09-07): *surface the limitation*, not six more Jinja families duplicating dialect knowledge `configgen.ts` already owns (duplicated engines have drifted every time: AB1, AB4, AB7b, AD3, AD4). New `config_support_matrix()` + `GET /api/config-support` state supported vendors (Cisco/Arista/Juniper/NVIDIA), the six with no template family and the layers they supply, the template families on disk, and where the full-coverage engine is — so a consumer can check **before** building a design instead of POSTing one to find out. `ConfigResponse` gains `unsupported` + `unsupported_note` so a caller branches on a field rather than pattern-matching config text. **Writing the matrix found that the thing it documents was unreachable.** `_detect_primary_vendor` reads `state["vendors"]`, and `DesignState` — the HTTP request model — **had no vendor field at all**, so through `/api/generate-configs` the detection always returned `""` and every design took the Cisco layer default: a Nokia design got NX-OS CLI, exactly the behaviour AB7 and AD4 spent two PRs stopping. The guard only ever fired for direct `generate_all_configs()` calls, which is what their tests did. Added `vendors` to `DesignState`; `_detect_primary_vendor` also accepts the frontend's `vendorPrefs` spelling and, failing all three, **infers the vendor from the selected SKU ids** (`nokia-srl-7220d3`, `dell-z9332f`) — a design is its hardware, so the guard holds for a caller that declares nothing. An omitted vendor still legitimately takes the layer default. 20 tests; reverting the fix trips 6 | [x] | `backend/config_gen.py` `config_support_matrix`/`ALL_CATALOGUE_VENDORS`/`_vendor_from_token`/`_detect_primary_vendor`, `main.py` `DesignState.vendors`/`ConfigResponse`/`GET /api/config-support`; `test_config_support_matrix.py` (20); backend 436→456 |
...
| AG10 | **Diagnostics now say which of your fleet they can actually serve** — user decision (2026-09-07): *surface the limitation*. The playbooks carry command variants for four NOSes (NX-OS, IOS-XE, EOS, JunOS); Nokia SR Linux, Extreme EXOS, NVIDIA Cumulus, Dell OS10, FortiOS, Aruba AOS-CX, PAN-OS and IOS-XR fell through to the Cisco base, so a Nokia operator investigating a down session was handed `show ip bgp summary`, which SR Linux does not have. New `lib/troubleshoot-coverage.ts` + `backend/platform_coverage.py` (one map each side, parity-tested) resolve the design's real NOSes through `ztpPlatform` — the AG5/AG6 single vendor map. The Troubleshoot tab states the gap **before** the operator copies a command ("Diagnostics cover 0 of 14 devices … 14 devices run Nokia SR Linux — the commands below are Cisco-family and will not run there", naming the devices), the platform selector is built from the shared list and **defaults to a NOS the fleet actually runs** instead of a hardcoded `nxos`, and `POST /api/troubleshoot` carries a `platform_coverage` block so an API consumer never has to infer it. **Two live defects surfaced while doing it.** (1) Both `_best_platform` helpers picked a NOS from a *use case* (`uc == 'gpu' → sonic`) — a use case is not a NOS, and the browser emits Cumulus NVUE for NVIDIA hardware (AB7b). (2) Worse: `monitor_engine` carries 87 nxos / 81 eos / 58 iosxe / 36 sonic command sets but exactly **one** junos set, and both engines labelled the fallback result with the platform that had been *asked for* — so a Juniper design presented NX-OS commands under a `junos` key, with no way for the reader to tell. New `resolve_commands()` returns `(actual_platform, commands)` and the dict is keyed by what the commands **are**. The engines keep their own wider `ENGINE_PLATFORMS` (incl. SONiC) rather than being flattened onto the playbook set, which would have silently dropped 36 real command sets. 7 frontend + 22 backend tests; reverting the vendor map trips 2 | [x] | `lib/troubleshoot-coverage.ts`, `Step6Deploy.tsx`, `backend/platform_coverage.py`, `troubleshoot.py`, `troubleshoot_engine.py`, `monitor_engine.py`, `main.py`; `troubleshoot-coverage.test.ts` (7), `test_troubleshoot_coverage_parity.py` (22); 1496 frontend / 436 backend |
...
| AG7 | **Policy overlays now state their own coverage** — user decision (2026-09-07): *surface the limitation* rather than write 21 policies × 6 vendors of renderers duplicating dialect knowledge `configgen.ts` already owns. `policies.ts` does the right thing where it cannot help — `render()` returns `null` rather than emitting another vendor's CLI (the AB7 principle) — but the skip was **silent**: measured per-device application rates are Cisco 100%, Arista 65%, NVIDIA 47%, Nokia/Dell 45%, Extreme 44%, **Juniper 38%**, so an operator ticking "Control-Plane Policing" on a Nokia fabric got it on the Cisco firewalls and on none of the leaves with nothing said. New exported `policyCoverage(policy, devices, useCase)` → `{eligible, covered, missingVendors}`, computed against the **actual BOM**, not a sample device. The Config Policy Library shows a per-policy chip — emerald at full coverage, amber partial, red when a ticked policy reaches **nothing** — plus "no template for Juniper, Nokia" naming the vendors, and a toolbar summary of every gap in the current selection. Deliberately not hidden behind an expander: the moment to learn a policy will not apply is while ticking it. 5 tests, incl. one that drives a real Juniper DC BOM and asserts the gap set is non-empty — so if renderers are ever added the test tells us the disclosure is now over-reporting | [x] | `lib/policies.ts` `policyCoverage`/`PolicyCoverage`, `ConfigPolicyModal.tsx`; `policies.test.ts` 11→16; 1489 tests, tsc + build green |
...
| # | Item | Status | Notes |
|---|------|--------|-------|
| AJ2 | **The compliance report contradicted the design it was handed** — the same measurement session that produced AJ1. Eight controls read Step-2 **form fields** and ignored `devices` and `configs` entirely, so on a generated DC design: PCI-1.1 / SOC2-CC6.6 / FDRP-SC-7 reported *"No firewall model selected"* on a BOM containing **two firewalls** — cabled, configured, with a border-leaf handoff (Z3); PCI-1.3 / HIPAA-164.312e / ISO-A.13.1 reported *"no segmentation"* on a **VXLAN/EVPN fabric carrying a TENANT-A VRF**; and HIPAA-164.312d hard-**FAILED** "network access authentication" on a spine-leaf fabric, which has no user ports to 802.1X — the applicable control there is device AAA, which every generated config has. That is the BOM↔config disagreement theme one layer up: a report that contradicts its own inputs. New `firewallControl` / `segmentationControl` read the design first and treat the form as corroboration, so the three states are now distinguishable: **pass** (present in the BOM/configs, naming the models or the construct found), **warn** (*requested* but absent — the discrepancy, previously invisible), **fail** (absent from both). `segmentationControl` is vendor-aware across all 7 fabric dialects (`vrf context`/`vrf instance`/`routing-instances`/`network-instance`/`nv set vrf`/`ip vrf`/`virtual-router`/NVE/Vxlan) and separately accepts **VLAN isolation** — PCI 1.3 names it explicitly — but requires ≥2 non-default VLAN ids, since every switch config mentions the word, and it *says* "L2 isolation only" so a reader scoping a CDE can tell it from VRF-grade separation. Scores: DC **60→83** with zero failures, campus **73→83**; GPU and WAN still fail PCI-1.1/FDRP-SC-7 because those BOMs genuinely have no firewall — an honest finding. Two pre-existing tests asserted the old behaviour (the fixture declared a perimeter firewall and a VXLAN overlay in its form while containing neither, and "detects missing firewall" meant "form field unset"); the fixture was the unrealistic part and now contains what it claims. 8 tests; reverting trips 4 | [x] | `lib/compliance-scan.ts` `firewallControl`/`segmentationControl`/`dataVlanIds`/`hasAccessPorts`/`RE_SEGMENTATION`/`RE_DOT1X`/`RE_ENCRYPTION`; `compliance-scan.test.ts` 33→41; 1517 tests, tsc + build green |
...
### AK. NETCONF panel fidelity (sourced 2026-09-15)

> Same sweep that produced AG10 and AJ1, now aimed at the NETCONF tab. Its own
> blurb states the limit — *"Supported on Juniper JunOS, Cisco IOS-XE 16.6+,
> Cisco NX-OS, Arista EOS"* — but the device dropdown lists **every device in
> the BOM**, so the product knows the boundary and ignores it.
>
> Measured: an **NVIDIA DC design offers NETCONF on 12 of 14** Cumulus boxes,
> which run NVUE REST and have no NETCONF server at all; a **Fortinet campus
> design on 18 of 18**; Palo Alto, Extreme and Aruba likewise. Picking one
> yields a Cisco-flavoured `ietf-interfaces` RPC that can never run.

| # | Item | Status | Notes |
|---|------|--------|-------|
| AK2 | **The Rollback Advisor recommended rolling back deployments that changed nothing** — and it was right to, given what it was fed. Two defects in the pre/post check simulator, which lived buried in a 6000-line component, which is why nobody had measured it. (1) **Each phase rolled its own dice**: `Math.random()` per check per phase, so the same device+check could be PASS in pre and FAIL in post by chance alone. Measured on 14 devices × 13 checks with **nothing deployed between the phases**: an average of **24.5 regressions per run** (range 21–34), ~7.8 of them **critical** PASS→FAIL. Every single run advised a rollback. A simulation of a network should model the network, not re-roll it — the baseline is now a stable FNV-1a hash of (device, check), so post differs from pre only where something actually changed, and an injected fault is the one thing that moves a status. The documented ~85/10/5 mix (§3) is preserved as a *property* rather than a coin flip, and a test pins the distribution so "stable" did not quietly become "uniform". (2) **Checks were role-blind**: all 13 ran on every device, so an access switch and a firewall reported *"2 BGP peers Established"*, and a DC fabric reported an **OSPF adjacency** for a protocol §6 rule 4 says it must not run (IS-IS for DC/GPU, OSPF for WAN/campus — never both). Checks now follow the device's role and the design's underlay, and the fabric gains an **IS-IS Adjacency** check it never had. `HEALTH_BGP_LAYERS` was a private const inside `HLDTopologyDiagram` while `monitoring.ts` owned the heuristic it overrides — hoisted to one exported `BGP_LAYERS` rather than writing a third copy (the AG5/AG6/AG10/AK1 consolidation, again). Extracted to `lib/checks-sim.ts` so it is testable at all. 9 tests, incl. 25 consecutive no-op runs asserting **zero** regressions, an injected-fault case asserting **exactly one**, and a sweep proving no role in any use case ever reports both underlays; reverting the determinism trips 3 and the role-awareness trips 4. Frontend-only — the backend suite was not runnable in this container (no pytest, pip timed out through the proxy) | [x] | `lib/checks-sim.ts` (new), `lib/monitoring.ts` `BGP_LAYERS`, `HLDTopologyDiagram.tsx`, `Step6Deploy.tsx`; `checks-sim.test.ts` (9); 1538 tests, tsc + build green |
| AK1 | **The NETCONF panel offered a protocol half the fleet does not speak** — its own blurb named four supported platforms while the device dropdown listed **every device in the BOM**, so the product knew the boundary and ignored it. Measured: an **NVIDIA DC design offered NETCONF on 12 of 14** Cumulus boxes (NVUE REST, no NETCONF server at all), a **Fortinet campus design on 18 of 18**, and Palo Alto / Extreme / Aruba likewise — each producing a Cisco-flavoured `ietf-interfaces` RPC that can never run. New `lib/netconf.ts` resolves capability through `ztpPlatform` (the same single vendor→NOS map AG5/AG6/AG10 consolidated onto — a fifth private vendor table is the drift this codebase keeps paying for) and carries a real profile per platform: datastores, whether a candidate edit needs an explicit commit, YANG flavour and a **genuine interface name**. **Four defects fixed, not one.** (1) Unsupported devices are now grouped, disabled and labelled with what they *do* speak ("FortiOS — use FortiOS REST API"), with a coverage notice above. (2) **NX-OS has no candidate datastore**, but the dropdown offered it on every device — selecting it returned an RPC error; the datastore list now comes from the profile. (3) **There was no `commit` operation at all**, so the panel's own workflow was a dead end: it offered edit-config to candidate and no way to apply it. `commit` now appears exactly where a candidate is committed, and a test asserts a platform cannot claim one without the other. (4) Only `edit-config` branched on Juniper, so a Juniper **get-config** asked for `ietf-interfaces` while its own edit used the `xnm` model — the panel disagreed with itself about one vendor — and every other platform got `GigabitEthernet1`, an interface name none of them have; each platform now uses its own model (`openconfig` for EOS, `srl_nokia` for SR Linux) end to end. Deleted the superseded 58-line inline builder rather than leaving dead code to mislead the next reader (the AH10 lesson). 12 tests incl. a both-maps-cover-every-vendor guard, a no-Cisco-interface-name sweep, and XML well-formedness checked with a **real parser** plus a guard that the check can fail — a hand-rolled tag-balance count trips over the `/` in namespace URLs and reports nonsense. Reverting the profiles trips 4 | [x] | `lib/netconf.ts` (new), `Step6Deploy.tsx`; `netconf.test.ts` (12); 1529 tests, tsc + build green |

### AL. Drift remediation dialect (sourced 2026-09-24)

> Swept the remaining demo engines after AK2. The `Math.random()` class is
> genuinely closed (the two surviving calls are a doc comment and a fake
> Ansible Tower job id, which should be random). But **drift remediation** —
> the commands generated to repair a device whose running config has drifted —
> is a **binary Junos-or-Cisco** split on BOTH sides of the stack.
>
> Measured across the catalogue: **8 of 10 vendors receive Cisco IOS `no`
> negation.** Four of those (Cisco, Arista, Dell OS10, Aruba AOS-CX) genuinely
> are IOS-style for this, but **Nokia SR Linux, NVIDIA Cumulus, Extreme EXOS,
> FortiOS and PAN-OS are not** — none of them accepts `no <line>`.
>
> This is the AG5 severity argument again: you run remediation when the device
> is already wrong. And the AG9 pattern — the backend `config_drift.py` carries
> the identical `_is_junos` / `_negate_cisco` split, so a key-only parity check
> passes with both sides wrong the same way.

| # | Item | Status | Notes |
|---|------|--------|-------|
| AL1 | **Drift remediation spoke Cisco to boxes that do not** — a **seventh** vendor map, and the AG5 severity argument again: remediation runs when the device is **already wrong**, so a bad command lands on a box someone is mid-way through fixing. `simulateRemediation` / `generate_remediation` were a binary Junos-or-Cisco split, so measured across the catalogue **8 of 10 vendors received Cisco IOS `no <line>` negation**. Four of those (Cisco, Arista, Dell OS10, Aruba AOS-CX) genuinely are IOS-style for this, but **Nokia SR Linux, NVIDIA Cumulus, Extreme EXOS, FortiOS and PAN-OS** are not. The backend carried the *identical* split — the AG9 pattern, where a key-only parity check passes because both sides are wrong the same way. Dialect now comes from AG8/AG9's shared `cliFamily`/`cli_family` rather than an eighth private table. **EXOS and FortiOS are deliberately left unsupported rather than guessed**: EXOS negation is per-command (`unconfigure`/`disable`/`delete` depending on what the line set) and FortiOS needs the enclosing `config` path to emit `unset`, so neither is derivable from a diff line — a plausible `unconfigure <line>` would look runnable and not be (the AG5 note-field principle). Those devices report `supported: false` with a note naming the real mechanism, surfaced in the UI instead of an empty command block. **I introduced a regression doing this and an existing test caught it**, which is worth recording: `cliFamily` exact-matches *catalogue vendor names*, but this module's field is literally called `platform` and also receives *NOS tokens* — so `juniper-junos` fell through to `ios` and **Juniper regressed to Cisco negation**. Both shapes are live (the UI passes `d.vendor`, the API passes a platform string), so new `remediationFamily`/`remediation_family` resolves either, and a 23-case parametrised test pins both vocabularies so it cannot come back. 7 frontend + 40 backend tests; reverting the dialect trips 5 and 6. The 9 `test_intent_ai.py` failures in this container are pre-existing (no `anthropic` SDK) — verified identical with the change stashed | [x] | `lib/drift-remediation.ts` (new), `Step6Deploy.tsx` (3 superseded helpers deleted), `types/index.ts`; `backend/config_drift.py`, `main.py` `RemediationDevice`; `drift-remediation.test.ts` (7), `test_drift_remediation_parity.py` (40); 1545 frontend / 487 backend |

### AM. Normalized config facts (sourced 2026-09-25)

> The one research lead with the strongest fit, because it targets the root
> cause of a defect class this codebase keeps paying for rather than a symptom.
> Every compliance/validation check carried its **own** regex listing every
> vendor's spelling of the thing it looked for; each missed dialect was a
> silent wrong answer, fixed one vendor at a time in M3, M4, M6, M7, Z6, AJ1
> and AL1. The fix is structural: extract a small set of dialect-free facts per
> device, once, and have checks query facts instead of text.

| # | Item | Status | Notes |
|---|------|--------|-------|
| AM1 | **Management-plane facts, measured wrong in both directions** — the compliance scanner's SSH / NTP / syslog / AAA checks were each one cross-vendor regex. Measured against every vendor's real generated config, they were wrong **both ways**. **False FAIL** (config correct, dialect unknown to the regex): NVIDIA Cumulus NTP `nv set service ntp mgmt server …` (12 of 14 devices), Palo Alto NTP `set deviceconfig system ntp-servers …`, HPE Aruba SSH `ssh server vrf mgmt` (16 of 18), FortiOS SSH `set admin-ssh-v1 disable`. **False PASS** (regex matched something that was not the fact): a Cisco FTD passed syslog on a **comment** (`!   syslog: logging host …` in its FMC manifest) — Z6 fixed comment-matching in the validator and **the fix never reached the compliance scanner** — and **Nokia SR Linux, NVIDIA Cumulus and HPE Aruba passed PCI-8.1 "AAA / RADIUS / TACACS+" because `/aaa\|radius\|tacacs/` matched the word `aaa` in a LOCAL admin-user block**; none has TACACS+ or RADIUS anywhere. "
"New `lib/config-facts.ts`: `extractFacts(config, platform)` → `{sshV2, ntp, syslog, aaa}`, each **tri-state** (`present` / `absent` / `unknown`) with the matching **config line as evidence**. (1) **One place per dialect**: `RULES` holds every platform's spelling of every fact, so a check no longer knows any vendor. (2) **Completeness is compiler-enforced**: `RULES` is a full `Record<FactPlatform, Record<FactName, Rule>>`, so adding a platform or fact without deciding every cell is a type error, not a silent `false`. (3) **`unknown` ≠ `absent`**: FTD syslog and AAA are set in FMC, not the device CLI, so they report *unverifiable, with the reason* instead of a failure the user cannot act on. (4) **Comments can never satisfy a fact** — extraction always strips them, so the Z6 rule cannot be forgotten by the next consumer (the validator's `stripComments` is exported and reused, not copied). (5) Patterns are anchored to lines the generators actually emit and deliberately narrow — the measurement showed broad patterns matching a Junos zone rule `system-services [ ssh ntp ]`, NX-OS `log-adjacency-changes` and FortiOS `ssl-ssh-profile` as if they were SSH/NTP/syslog. Platforms resolve via `ztpPlatform` (the single vendor map) plus FTD, which it folds into IOS-XE but whose CLI is not IOS-XE. "
"Seven compliance call sites now route through one `everyDeviceFact()` (PCI-2.3, FDRP-AC-17, PCI-6.1, FDRP-AU-2, PCI-8.1, PCI-10.1, HIPAA-164.312d), per device like AJ1, and a pass cites a real line. The three superseded regexes and the regex `everyDeviceHas` are deleted. **Two tests encoded the defect**: the `BASE_STATE` fixture gave an NX-OS spine and a *Firepower* IOS syntax, which the dialect-blind regex passed; and AJ1's "all vendors score the same" held only because AAA false-passed Nokia/Cumulus. The fixture now speaks each device's real dialect, and the vendor test asserts the stronger true property — every control except PCI-8.1 is vendor-independent, and PCI-8.1 matches verified per-vendor ground truth. **An extractor bug my own test caught**: evidence returned only the matched prefix (`ntp server`), useless to an auditor; it now returns the full line. 10 new tests; reverting to the broad AAA regex trips 3, skipping comment-stripping trips 1 | [x] | `lib/config-facts.ts` (new), `lib/compliance-scan.ts` `everyDeviceFact`, `lib/config-validator.ts` (`stripComments` exported); `config-facts.test.ts` (10), `compliance-scan.test.ts` (fixture + vendor test rewritten); 1555 tests, tsc + build green |
| AM2 | **The validator's management checks read the facts — and V-07 had been passing almost everything** — V-06/V-07 had their own detectors (`RE_HOSTNAME`, `RE_MGMT`) in parallel with the compliance scanner's facts: two detectors for one fact, which is how they drift. Replacing them showed `RE_MGMT` was not a detector at all: its first alternative was the bare word `MANAGEMENT`, so a comment banner or any `vrf management` line satisfied it, and it accepted **any one** of NTP, syslog or SNMP. V-07 now requires NTP **and** remote syslog per device, in that device's dialect, and names which one is missing; `unknown` facts (FTD syslog in FMC, O-RU PTP time) are not counted as gaps. New `hostname` fact (16 dialects: Junos `set system host-name`, EXOS `configure snmp sysName`, PAN-OS `set deviceconfig system hostname`, FTD `configure network hostname`, …) drives V-06. A config that cannot be matched to a BOM device is read by `extractFactsAnyDialect` — lenient, and it never attributes a dialect — rather than a guessed one. **The switch found a real generator bug**: Nokia SR Linux emitted `system { hostname X }`, which SR Linux rejects (it is `system { name { host-name X } }`); the cross-vendor `/hostname/` regex had accepted it. Comment handling moved to a shared `lib/config-text.ts` (it was defined in the validator and copied, with different rules, into `ipam-truth.test.ts`), which also removes the facts↔validator import cycle. 4 new tests; reverting trips all 4 | [x] | `lib/config-validator.ts` V-06/V-07, `lib/config-facts.ts` `hostname`/`extractFactsAnyDialect`, `lib/config-text.ts` (new), `lib/configgen.ts` SRL; `config-validator.test.ts` 57→61; 1563 tests, tsc + build green |
| AM3 | **Routing and fabric checks read the facts — and measuring them found six generator bugs** — new `ROUTING_RULES` (bgp / isis / ospf / loopback / vxlan / evpn / bfd / jumboMtu × 16 dialects, compiler-complete like `RULES`), and V-01/V-03/V-08/V-12/V-13/V-14 now query one per-validation `FactMap`; `RE_BGP`/`RE_ISIS`/`RE_OSPF`/`RE_ROUTING_DEVICE`/`RE_LOOPBACK_IFACE` and the inline VXLAN/BFD/MTU regexes are gone, so no check in the validator knows any vendor's syntax. Diffing old detectors against the new facts on every vendor × use case showed the old regexes were mostly well-tuned by M3–M9, with two real misses: **EXOS routers were never classed as routing devices** (`enable bgp` matched no detector), so V-12 never asked an EXOS spine or leaf for a loopback; and the loopback detector matched `interface Loopback0` **inside** `logging source-interface Loopback0`. That second miss led to a new measurement — **every interface a config sources a service from must exist in that config** — which found real defects: (1) **IOS-XE access switches** sourced NTP/syslog from a `Loopback0` they never create; (2) **`mgmtBlock` configured NTP, syslog and SNMP twice** — AA1 added a Loopback0-sourced section without noticing the Vlan-sourced originals below — and the **ISR firewall and WAN router** that use it defined *neither* interface; now one explicit `src` parameter, and those routers gain a real `Loopback0`; (3) **Arista campus switches used the IOS-XE `mgmtBlock`** — `aaa new-model`, a `tacacs server NAME` block, `ip http secure-server`, `login block-for`, all rejected by EOS — sourced from an undefined `Vlan10` and with `hostname` twice; the spine's correct EOS block is now a shared `eosMgmtBlock()` used by spine, leaf and campus (the leaf had its own copy); (4) the **O-RAN fronthaul** sourced SSH from an undefined `mgmt0` (an AM4 regression of mine) and lacked `feature interface-vlan` for its management SVI — now everything sources from that SVI. A new cross-vendor fabric matrix then found (5) the **Dell OS10 spine configured itself as a VTEP** under a comment saying it is not one, and every Dell leaf's VXLAN was inert: `vxlan-vni` sat under `interface virtual-network` (the IRB) instead of `virtual-network N`, with no `nve` source at all; and (6) the **Extreme EXOS spine** likewise built a VNI while no EXOS leaf set `configure virtual-network local-endpoint`, so there was no tunnel source. Also replaced three hard-coded `8.8.8.8` DNS servers with a placeholder. New `test/source-interface.test.ts` (4) + 3 fact tests + 2 validator + 2 configgen; reverting the generator trips 6, the validator 2. **Not migrated:** V-09 (GPU QoS) still scans text | [x] | `lib/config-facts.ts` `ROUTING_RULES`, `lib/config-validator.ts`, `lib/configgen.ts` (`mgmtBlock`/`eosMgmtBlock`, ISR/WAN/generic, Arista campus, O-RAN fronthaul, Dell, EXOS); 1574 tests, tsc + build green |
| AM4 | **Fix the real config gaps AM1 surfaced — and the ones it could not yet see** — AM1 pinned no centralized AAA on Dell OS10, FortiOS, Juniper SRX + MX204, the ASR 1002-HX cEdge and local-only on Nokia SRL / Cumulus / Aruba, plus no SSH hardening on PAN-OS. Added TACACS+ in each platform's real dialect (all `<CHANGE-ME-*>`): Dell `tacacs-server host … vrf management` + `aaa authentication login default group tacacs+ local`; Aruba `aaa group server tacacs` in `vrf mgmt`; Cumulus `nv set system aaa tacacs …` + authentication-order; SR Linux `server-group TACACS type tacacs` + `authentication-method [ TACACS local ]`; FortiOS/FortiSwitch `config user tacacs+` + remote-auth wildcard admin; Junos SRX/MX `set system tacplus-server`; cEdge `tacacs server` + `aaa group server tacacs+` in `Mgmt-intf`; PAN-OS binds `deviceconfig system authentication-profile` and disables Telnet/HTTP. **Sweeping every use case (AM1 only measured dc/campus) found more.** (1) **Every Juniper switch had broken AAA**: `set access tacacs-server` is not a Junos statement, so `authentication-order [ tacplus password ]` pointed at no server and every login silently fell through to the local password — replaced with `set system tacplus-server` on all three generators. (2) The **vEdge** said `auth-order local radius` with no RADIUS server, which the old `/radius/` match counted as central AAA — now a real `tacacs server … vpn 512`. (3) The O-RAN **fronthaul** NX-OS switch still had the X1 IOS-ism `username admin privilege 15` and no SSH hardening or AAA; the **midhaul** IOS-XR router had no AAA. (4) Facts **model** gaps, fixed rather than papered over: IOS-XR accepts flat `ntp server` (the rule demanded the nested form); vEdge runs Viptela OS, not IOS-XE — new `viptela` dialect; O-RAN CU/DU/UPF/grandmaster were resolved through the *server vendor* (Dell OS10 / IOS-XE rules) and false-FAILED all four facts — new `oran-nf` and `oran-ru` dialects, with the RU's NTP and AAA declared `unknown` with the reason (PTP timing; O-RAN WG4 NACM accounts, no TACACS+ client) instead of invented config. The ground-truth test now covers 6 use cases and asserts **no generated device on any platform lacks a management fact**; the compliance vendor test asserts PCI-8.1 passes for every vendor. 4 new tests; reverting the generators trips 3 | [x] | `lib/configgen.ts` (13 generators), `lib/config-facts.ts` `viptela`/`oran-nf`/`oran-ru`; `config-facts.test.ts` 10→14; 1559 tests, tsc + build green |
| AM5 | **HPE Aruba DC fabric: it had no leaves, and nothing else worked either** — AM3's dump showed placeholders; building it showed the root cause was one tier up. **The BOM had no Aruba leaves at all**: `buildDeviceList` took `subLayer` from the SKU's *catalogue* role rather than the slot it filled, and the CX 6400 is catalogued as `distribution`, so every Aruba DC and multisite leaf came out as a distribution switch — invisible to fabric links, VTEPs, the border-leaf handoff and the HLD, and skipped by leaf uplink sizing. The slot now decides the tier (only Aruba was affected; a new test asserts `subLayer === role` for every device across 10 vendors × 7 use cases). The generator then got the Z8 treatment it had missed: `arubaFabricConfig` (tier-scoped identity instead of one `<CHANGE-ME-asn>` / `<CHANGE-ME-loopback-ip>` on every device, `closFabricLinks` /31s with jumbo MTU, eBGP IPv4 underlay per /31 plus eBGP EVPN overlay between loopbacks, spine `next-hop-unchanged`, BFD, leaf-only VTEP with explicit RTs, server ports, border-leaf `vrf TENANT-A` handoff) and `arubaCampusConfig` (distribution had shared the fabric BGP/EVPN placeholder — now OSPF + VRRP; access switches had **no uplinks**, so their VLANs reached nothing — now split uplinks to the pair). Shared `arubaMgmtBlock` pins SNMP, syslog and NTP to `vrf mgmt` (they were in the default VRF, which has no route to OOB) and drops an invalid `no interface mgmt shutdown`. CX 6400 gains `uplinkStart: 49`. **Aruba had never been in the e2e `VENDOR_SETS`**, which is how it stayed behind — enrolled there, in `HANDOFF_VENDORS`, the AM3 fabric matrix and the AM6 underlay test. Its first full-harness run then caught a validator false-FAIL: V-05 read AOS-CX's encoding keyword (`password ciphertext <CHANGE-ME-…>`, `key plaintext <CHANGE-ME-…>`) as the secret itself, failing every correctly placeholdered Aruba device; the keyword is now skipped and may never be taken as the value. 11 new tests; reverting trips 12 | [x] | `lib/bom.ts` (`subLayer: role`), `lib/products.ts` CX 6400, `lib/configgen.ts` `arubaMgmtBlock`/`arubaFabricConfig`/`arubaCampusConfig`, `lib/config-validator.ts` V-05; 1587 tests, tsc + build green |
| AM6 | **Dell OS10 and Extreme EXOS fabrics had no underlay — no session could ever come up** — found reading Dell's source while scoping AM5. Its comment read *"UNDERLAY: eBGP over the /31s, overlay eBGP over loopback 0"*, but only the loopback-to-loopback overlay sessions were emitted. With no IGP and no session on the /31 links, nothing ever advertised a loopback, so every overlay session pointed at an address with no route. EXOS had the identical shape, and its spine-side overlay sessions also lacked `source-interface vlan Loopback0`. Z8 gave both vendors real identity and real peers but not the layer underneath them, and every existing test checked that sessions were *configured*, not that they were *reachable*. Fix: `closFabricLinks` now returns each link's far-end `peerIp` and `peerIdx`, and both generators emit one IPv4 eBGP session per fabric /31 carrying the loopbacks (Dell `neighbor <p2p> / address-family ipv4 unicast activate`; EXOS `configure bgp add network <lo0>/32` + `add neighbor <p2p>`). Validator: V-04 collected device addresses only from `ip address X`, so EXOS's `ipaddress X MASK` read as phantom peers — now both forms. New permanent test across all 7 fabric vendors: **every spine and leaf has an underlay** (IGP, BGP unnumbered, or a session to its own /31 far ends), and every underlay neighbor is an address another device really configures, with exactly one session per link end; reverting trips both (24 devices) | [x] | `lib/configgen.ts` `closFabricLinks` `peerIp`/`peerIdx`, `dellOs10SwitchConfig`, `extremeExosConfig`; `lib/config-validator.ts` V-04; `configgen.test.ts` 256→258; 1576 tests, tsc + build green |
...
| # | Item | Status | Notes |
|---|------|--------|-------|
| AH13 | **Light-mode contrast, and a probe worth trusting** — AH12 deliberately left this unverified because two attempts at the measurement disagreed. The bug in both was the same: **never parse alpha textually**. Tailwind v4 emits `oklch(L C H / .03)`, so an alpha regex written for `rgba()` reads a 3%-white card overlay as *opaque white* and inverts every result on the page. The fix is to stop parsing entirely — collect every ancestor background from the root down and let the canvas composite them, which is what the browser does whatever the syntax — plus a **sanity assertion** (a page heading must score high) so a broken probe fails loudly instead of reporting plausible nonsense. Cross-validated: two independently written probes now agree dark is 0. Light mode then measured **28 nodes across 8 styles below AA**, worst of all the selected use-case card's description at **1.24:1** — `text-blue-200/70` on the pale-blue selected ground, effectively invisible. Fixed by using a gray token (the `html.light` block already remaps the ramp; a fixed blue cannot follow it) and by darkening the light ramp's `gray-500`/`gray-600`. Both themes now measure 0 | [x] | `index.css` light ramp, `Step1UseCase.tsx`; 1484 tests, tsc + build green |
| AH12 | **Muted body text failed the contrast floor** — measured in a real browser by resolving every text node's colour and its composited background through the canvas (a hand-rolled parser reads Tailwind v4's `oklch()` channels as RGB and produces nonsense — the first run of this probe did exactly that and had to be thrown away). On the dark theme **28 text nodes across 8 distinct styles were under WCAG AA 4.5:1**: `text-gray-600` at **2.46:1** and `text-gray-500` at **4.02–4.19:1**. Those two tokens carry 343 text nodes between them — descriptions, hints, section labels, the breadcrumb — so retuning the tokens fixes all of them at once, and the ~15 non-text uses (a border, a few backgrounds) move one perceptual step, which is not noticeable. Values chosen by measuring, not by eye: `gray-500 → #8b94a3`, `gray-600 → #78828f`, landing on 0 failures. Revert-checked: putting the old values back returns exactly 28. **Not claimed: light mode.** Its probe needs correct alpha compositing of dark tints over a light ground, and two attempts at that gave contradictory numbers, so the light theme is unverified and left for its own pass rather than reported on | [x] | `index.css` `--color-gray-500`/`--color-gray-600`; 1484 tests, tsc + build green |
| AH11 | **Keyboard focus was invisible across the whole app** — WCAG 2.4.7 (Focus Visible, AA), and a hard failure rather than a preference. Tailwind's preflight removes the UA outline and the codebase adds `focus:outline-none` in places without a replacement, so tabbing through the app showed **6 of 12 focusable elements with no outline and no ring**; a sampled button changed *nothing* on focus — `outline: none`, `box-shadow: none`, border unchanged at `white/0.1`. One global `:focus-visible` rule in `index.css` fixes every control at once and is the version a new component cannot forget; `outline` rather than a box-shadow ring because it follows `border-radius`, never shifts layout, and is not clipped by an ancestor's `overflow: hidden`. Darkened on the light theme, where sky-400 cannot reach 3:1. **Method note worth keeping**: the first measurement used programmatic `.focus()`, which does not trigger `:focus-visible` — it reported 14/14 broken both before *and* after the fix. Re-measured with real `Tab` presses, before and after, which is the only valid probe. jsdom applies no stylesheets so the rule's effect cannot be unit-tested; instead the test pins the thing that broke it — no component may kill the outline without drawing its own indicator — and it caught a real offender (a Step 3 textarea) on its first run. Also swapped the last two sidebar emoji (`🔬`, `☰`) for glyphs | [x] | `index.css`, `Step3Config.tsx`, `Sidebar.tsx`, `icons` `IconMenu`; `focus-visible.test.tsx` (2); 1484 tests, tsc + build green |
| AH10 | **The exported diagrams no longer depend on emoji fonts** — the strongest argument for this whole group turned out not to be aesthetic. The HLD and LLD are **exported as SVG into design documents**, and an emoji in a `<text>` element resolves against whatever font the *viewing* application has — so it renders as tofu in Illustrator, Visio and most PDF pipelines. The zone swatches were coloured-circle emoji (`🔴🟠🔵`) that could only approximate the zone's own stroke; they are now real `<circle>` marks taking that exact colour. The internet node's `🌐` became a drawn glyph, and the `⚡ NetDesign AI` watermark plain text. Also removed the LLD's `icon` field entirely — AH4 switched rendering to `tierIcon(tier)`, so all 61 emoji entries had become **dead data** that would mislead the next reader. New permanent invariant across both diagrams × 5 use cases: **no emoji may appear in the rendered SVG** — it caught the `⚡` watermark that eyeballing the diagram had missed | [x] | `HLDTopologyDiagram.tsx`, `LLDTopologyDiagram.tsx`; `HLDTopologyDiagram.test.tsx` 33→39, `LLDTopologyDiagram.test.tsx` 9→13; 1482 tests, tsc + build green |
| AH9 | **One close control, with a name** — eight dismiss buttons written inline across the modals and the two diagram inspector panels, using `✕`, `×` or `X` at three different sizes. Four had **no accessible name at all**: a button whose only content is a glyph announces as that glyph or as nothing, so a screen-reader user could not tell what it closed. They were also bare text with no padding — roughly a 12px tap target. New `ui/CloseButton` gives every one a 32px hit area, a hover surface, a focus ring, and a name that says *what* is being closed ("Close My Designs", "Close device details"). One HLD test selected the button by its `✕` glyph; it now targets the accessible name, which is the point of the change | [x] | `ui/CloseButton.tsx` (new), `icons` `IconClose`, 7 components; 1472 tests, tsc + build green |
| AH8 | **Empty states that say what to do next** — a dozen panels rendered bare grey text, or a 4xl emoji over a sentence, and the Config Gen page showed a **full-height blank rectangle** across most of the viewport when nothing was selected. An empty panel is the moment a user does not know what to do, so it should say three things: what is missing, why, and the action that fixes it — enterprise tools get judged on this more than on the populated state, which people only reach if they get past it. New `ui/EmptyState` (glyph in a tile, headline, one explanatory sentence, optional action) applied to the Config Gen device list and config pane, the cabling and optics tabs, My Designs and Approvals. Each description is written in the user's terms, not the code's: "Transceivers are chosen from the cable runs — their length and fibre type" rather than "No optics data". Verified the populated state still renders — 18 configs, device list, CodeMirror — so the new guard cannot hide real content. 3 tests | [x] | `ui/EmptyState.tsx` (new), `Step3Config.tsx`, `Step2Design.tsx`, `MyDesigns.tsx`, `EnterpriseApprovals.tsx`; 1472 tests, tsc + build green |
| AH7 | **Landing page** — the first surface a visitor sees carried 11 emoji: six feature cards (`🧩 🏆 📐 🛡️ 🧪 📤`), six use-case chips and four hero pills. All now use the icon set, with the feature glyphs in a tinted rounded tile so the card has a real entry point rather than a floating pictograph. This is the page that decides whether the product looks credible before anyone clicks anything | [x] | `LandingPage.tsx`; 1469 tests, tsc + build green |
| AH6 | **Card titles carry a glyph, not an emoji in the string** — section headings were written as `<CardTitle>🛟 Rollback Advisor</CardTitle>`, so the pictograph sized and coloured itself independently of the heading it belonged to. `CardTitle` gains an optional `Icon` prop and the emoji-led titles across Step 6 use it (Enterprise ZTP Plan, Rollback Advisor, Active Alerts, Scheduled Scans, Closed-Loop Remediation, Push Incremental Change), plus the Run Diagnostics and Open Grafana buttons. **Deliberately left alone**: the deploy terminal's log lines (`✔ SSH reachability`, `⛔ Canary rollout aborted`) — a CLI-style log is exactly where those marks belong — and typographic arrows like `⬇ Download` | [x] | `ui/Card.tsx` `CardTitle`, `Step6Deploy.tsx`; 1469 tests, tsc + build green |
| AH5 | **Shared page chrome — one header and one stat tile** — the five step pages each wrote their own title block: Step 1 at `text-xl font-bold tracking-tight`, the rest at `text-lg font-semibold`, and the ones with actions laid them out ad hoc, so the product read as several products. New `ui/PageHeader` (title + description + right-aligned actions slot) used by all five. The summary rows were centred numbers on a flat card — label below value in the same visual weight, so the eye had no entry point and the row read as three unrelated boxes. New `ui/StatCard` leads with the label, left-aligns, adds a hairline colour accent and an optional device glyph, and uses tabular figures. 6 tests pin the contract the pages rely on, not the styling | [x] | `ui/PageHeader.tsx`, `ui/StatCard.tsx` (new), Step1/2/3/4/6; `page-chrome.test.tsx` (6); 1469 tests, tsc + build green |
| AH4 | **Device glyphs in the HLD and LLD diagrams** — the diagrams are the artefact people export into design documents, and every node was an identical rounded rect, so the reader worked out the tier from the text every time. The LLD additionally drew a per-node **emoji** (`🛡`, `🔀`, `🌐`) that rendered at a different size on every OS and could not take the node's colour. Both now draw the shared icon set inside the node box, tinted with the node's own border colour — so the BOM table and both diagrams agree on what a spine looks like. New `tierIcon()` maps the LLD's wider vocabulary (dmz, loadbalancer, database, endpoint, oob…) explicitly, since falling back to a generic switch for those would be worse than the emoji it replaces. Also migrated Step 4's tab row — a **fourth** tab style AH3 missed, with emoji in its labels — onto `TabBar` | [x] | `HLDTopologyDiagram.tsx`, `LLDTopologyDiagram.tsx`, `icons/index.tsx` `tierIcon`, `Step4NetworkDesign.tsx`; 1463 tests, tsc + build green |
| AH3 | **One tab control instead of three** — the app had three looks for one interaction: the Step 6 segmented control, bordered standalone buttons for the BOM sub-tabs, and an underlined row for the observability views. Worse than inconsistent: **only Step 6 had arrow-key navigation or complete ARIA**, so two of the three were not keyboard-reachable the way a tablist is expected to be. New `ui/TabBar.tsx` implements the WAI-ARIA tabs pattern properly — roving tabindex, Arrow/Home/End, focus following selection, `aria-controls` wiring, optional icon and badge — and all three rows now use it. (L1 deleted an unused `ui/Tabs.tsx` as dead code; this one is actually used, in three places.) 7 tests pin the keyboard behaviour so a future tab row cannot quietly lose it | [x] | `components/ui/TabBar.tsx` (new), `Step2Design.tsx`, `Step6Deploy.tsx`; `tabbar.test.tsx` (7); 1463 tests, tsc + build green |
