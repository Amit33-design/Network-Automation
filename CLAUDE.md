...
| AM2 | **Migrate the validator's management checks onto facts** — `config-validator.ts` detects NTP/syslog a *second* time in `RE_MGMT` (plus `RE_HOSTNAME`), independently of the compliance scanner's facts. Two detectors for one fact is how they drift apart; V-06/V-07 should query `extractFacts` too | [ ] | |
