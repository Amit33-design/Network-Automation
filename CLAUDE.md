...
| AA3 | WAN site-count coherence — an 8-site WAN design produced 6 edge routers, so site count and device count disagree; and `wan-edge↔wan-edge` currently models one circuit per router rather than a real hub-spoke / full-mesh topology driven by `dcTopology` | [ ] | `bom.ts` `buildDeviceList`/`buildCabling` |
| AA4 | The multisite `wan-edge→spine` DCI link is billed at **1G** (`GLC-LH-SMD`) because the vEdge 2000 SKU's `speed` is 1G — Z2's `min()` is behaving correctly on questionable SKU data. Either the SKU speed is wrong for the platform or the DCI should use an `uplinkSpeed`; a 1G DCI in front of a 36-leaf fabric is not a credible design | [ ] | `products.ts` (vEdge 2000) / `bom.ts` `linkSpeed` |
