...
| Z3b | Border-leaf firewall handoff parity for the remaining fabric vendors — the Z3 e2e invariant (8a⁗) proved Nokia SR Linux, NVIDIA Cumulus, Dell OS10 and Extreme EXOS leaves configured **no** handoff at all, so their designs billed firewall cables that terminated on nothing. Closed for **Nokia / Dell / Extreme** as part of Z8 (each now has a tenant VRF + routed /31 handoff + VRF default toward the firewall, and each is in `TENANT_VRF_VENDORS` in `e2e-journey.test.ts`). **NVIDIA Cumulus remains open**: it is a pure eBGP L3 GPU fabric with no tenant VRF at all (Y6), so it needs its own VRF/handoff story | [ ] | `configgen.ts` `nvidiaSpectrumConfig` |
