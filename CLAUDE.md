...
| AM5 | **HPE Aruba DC fabric is still in the pre-Z8 state** — found by AM3's dump: the Aruba AOS-CX spine emits `router bgp <CHANGE-ME-asn>` and `ip address <CHANGE-ME-loopback-ip>` (the same placeholder on every device, so eBGP could not form even once filled in), and has no BFD or jumbo MTU. Z8 fixed exactly this for Nokia/Dell/Extreme; Aruba was never in the e2e `VENDOR_SETS`, so nothing caught it. Needs the standard treatment (auto identity, `closFabricLinks` /31s, real per-leaf eBGP, next-hop preservation, BFD, jumbo MTU, border-leaf handoff) and Aruba added to the fabric parity matrix | [ ] | |
