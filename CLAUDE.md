...
| AA2 | Multicloud designs contain **only** cloud gateways/transit VPCs — an 800-endpoint multicloud design has no on-prem DC edge at all, so there is nothing for those 800 endpoints to attach to, and the design produces no deliverable artifact beyond a device list. Decide whether `buildDeviceList` should inject an on-prem edge tier for multicloud/aviatrix, and whether these use cases should emit Terraform rather than CLI | [ ] | `bom.ts` `buildDeviceList`; `lib/` new cloud-config generator |
...
| AB3 | Remaining pairs still unaudited: config generation (`lib/configgen.ts` ⇄ `backend/config_gen.py`), drift remediation (`lib/` ⇄ `backend/config_drift.py`), RCA (`lib/rca.ts` ⇄ `backend/rca/engine.py`). Config-gen is the big one — the TS engine covers 7 fabric vendors and the Jinja2 templates cover far fewer, so this is a capability gap rather than a drift gap, and needs scoping before it is worth a parity test | [ ] | `backend/config_gen.py`, `backend/config_drift.py`, `backend/rca/engine.py` |
