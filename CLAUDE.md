...
| AM4 | **Fix the real config gaps AM1 surfaced** — no centralized AAA (TACACS+/RADIUS) on Dell OS10, FortiOS, Juniper SRX + MX204, the Cisco ASR 1002-HX SD-WAN cEdge, and local-only on Nokia SR Linux / NVIDIA Cumulus / HPE Aruba; no SSH hardening statement on PAN-OS. These were previously **hidden** by false passes. They are generator defects, deliberately not fixed in AM1 so the facts model could not be tuned to hide them; the ground-truth test pins them and will flip as each is fixed | [ ] | |
