...
| AB7b | Write the missing backend template families so `/api/generate-configs` covers what the browser does: Nokia SRL, NVIDIA Cumulus (the current `sonic` templates are a **different NOS** — the frontend emits NVUE per Y6, so NVIDIA is a live mismatch even though it is nominally 'supported'), Dell OS10, Extreme EXOS, Fortinet, HPE Aruba, Palo Alto. Each should land with a parity test against the TS engine, or the drift starts immediately | [ ] | `backend/templates/`, `backend/config_gen.py` |
