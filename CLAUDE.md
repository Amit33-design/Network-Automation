...
| X5 | vPC / MLAG peer-link fully generated (not commented) + BOM optics completeness (host-facing DACs, firewall-uplink optics) + **GPU spine-count sanity**. GPU analysis (2026-07-10): `computeSpineLeaf` sets `spinesByUplinks = rawUplinksNeeded = ceil(downlinks×bw / oversub / spineSpeed)` and uses it directly as the spine count — at 1:1 with equal leaf/spine speed this equals the downlink count, so 512 GPUs @400G/1:1 (NVIDIA sn4600c leaf) produced **56 spines** (should be ~8). The obvious fix — cap `uplinksPerLeaf = min(rawUplinksNeeded, leafSku.uplinks)` and derive spineCount from it — correctly fixes NVIDIA (56→8, sn4600c has 8 uplinks) and passes the full e2e harness, BUT exposes that the **default Cisco/Arista GPU leaves (NX-9332C / 7050CX3) have only 2 uplink ports** → those fabrics cap to 2 spines for 2048 GPUs (SKU-honest but under-built), and the spine **fan-out** constraint (a spine must have ≥ leafCount ports per plane, else add spine planes) isn't modeled. Proper fix = cap-by-uplinks + choose adequate GPU-leaf SKUs (≥8 uplinks) + model multi-plane fan-out; then update ~20 `bom.test.ts` GPU spine assertions (they encode the old un-capped values). Attempted the cap 2026-07-10, reverted to keep it a focused increment | [ ] | `bom.ts` `computeSpineLeaf` + `products.ts` GPU leaf uplinks + `bom.test.ts` |
