...
| AA2 | Multicloud designs contain **only** cloud gateways/transit VPCs — an 800-endpoint multicloud design has no on-prem DC edge, so there is nothing for those endpoints to attach to. **The "emit Terraform rather than CLI" half is answered and shipped (AB5).** What remains is the BOM question: should `buildDeviceList` inject an on-prem edge tier for multicloud/aviatrix? That changes what those designs cost, so it needs a product decision rather than a default | [ ] | `bom.ts` `buildDeviceList` |
...
| AB4 | The 11 playbooks that pre-dated AB1 exist on both sides with independently-written content (different wording, step counts and commands). AB1's test asserts key parity only. Decide whether content should converge — most likely by making one side the source of truth and generating the other, rather than hand-syncing prose | [ ] | `backend/troubleshoot.py`, `frontend/src/pages/Step6Deploy.tsx` |
