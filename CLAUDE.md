...
| X7 | vPC / MLAG data-plane completeness + BOM optics completeness — (a) NX-OS vPC peer-link + Arista MLAG peer-link port-channels are emitted as comments (never initialize); generate real member interfaces + port-channels from the BOM port plan; Arista MLAG pairs also need a shared VTEP (`ip address virtual` / common Loopback1) or multihomed traffic is black-holed; (b) `buildOptics` covers only spine→leaf runs — add host-facing DACs (leaf→server) and firewall/border uplink optics so the optics BOM matches the full cable schedule | [ ] | `configgen.ts` (vPC/MLAG blocks) + `bom.ts` `buildOptics` |
