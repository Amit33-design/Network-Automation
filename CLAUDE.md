...
| AB6 | The other four export endpoints (`ansible`, `drawio`, `runbook`, `runbook/pdf`) are still unreachable from the product. Decide per endpoint whether to mirror client-side (the §3 pattern) or call the API in live mode — the runbook PDF in particular cannot be produced in the browser, so it may be the one genuine live-only export | [ ] | `backend/routers/export.py`, `backend/export/*.py` |
