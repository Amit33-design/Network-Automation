...
| Z2 | **BOM↔config physical honesty** (both reviewers, independently the #2 theme) — (a) link speed/optics take the **spine** SKU instead of `min(spine, leaf)`: dc-arista bills `QSFP-DD-400G-SR4` for a 7050CX3 whose cages are QSFP28-100G (a 400G module doesn't fit), same for QFX5120 and SN4600C; `validateBOM` proves the halves disagree — it rates the same link at 100G ("Effective ratio 7.0:1" only computes at 100G) while cabling rated it 400G; (b) no **breakout/channelization** config anywhere, so an 800G OSFP SN5600 port feeding a 100G leaf won't link (SN5600 cages are OSFP, not QSFP-DD); (c) BOM **omits real links** — vPC/MLAG peer-links (25 pairs × 2 = 50 links on Cisco, 44 on Arista) and campus peer-link/core-uplink have no cables/optics; (d) BOM **bills phantom links** — `leaf->gpu-compute qty=640` exceeds both leaf downlink supply (10×56=560) and server NIC ports (64×4=256); 40G optics ordered for a Firepower 4145 whose cages are 8×1/10G SFP+; (e) **spine port exhaustion silently truncates** — the 36-port 9336C emits 33 fabric + 3 FW handoffs = 36/36 and drops `IAD-FW-B02`, so ~10 of 24 cabled FW links land on unconfigured ports while the BOM still bills them. Fix = `min(spine,leaf)` speed, breakout emission, peer-link/host cabling in `buildCabling`/`buildOptics`, real host-link quantities, and cap-vs-truncate as a `validateBOM` **error**. Invariant to add: cable qty ≤ available ports AND == Σ configured interfaces | [ ] | `bom.ts` `buildCabling`/`buildOptics`/`validateBOM` + `configgen.ts` breakout |
| Z3 | Firewall is cabled to **spines** but a spine is not a VTEP and has no tenant VRF — architecturally it cannot route into `TENANT-A` (XC-2). The handoff /31s are also in no IGP (`no ip router isis` / no `isis enable`), and no default route is ever originated into the fabric, so north-south traffic cannot work in any design. Fix = move the FW attachment to **border leaves** (new `border-leaf` role or designate 2 leaves) with the VRF + route-leak, put the handoff in the IGP, and originate a default. Also: campus dist has no default route toward the FW and the handoff /31s are excluded from OSPF | [ ] | `bom.ts` (border-leaf role) + `configgen.ts` (handoff in IGP, default-originate) |
| Z4 | Campus physical-layer correctness — the C9500-48Y4C has **48×25G SFP28 (`TwentyFiveGigE1/0/x`) + 4×100G (`HundredGigE1/0/49-52`)**; every dist data-plane command uses `TenGigabitEthernet1/0/x`, which **does not exist on the platform** → all trunks, the core uplink, the FW handoff, the peer-link AND the HSRP tracked object are rejected (C-5). The C9200-48P access uplinks are the 4-port SFP module (`Te1/1/1-4`), not front-panel `Gi1/0/47-48` copper (C-6) — and the BOM buys 25G SFP28 optics for links a 9200 can't terminate. The Y2 `uplinkStart` mechanism was never given campus parity. Also C-7: `ip default-gateway 10.255.99.254` is owned by nobody (dist SVIs are .1/.2, Vlan99 has no HSRP) and 10.255.99.0/24 is in no OSPF network statement → mgmt + **RADIUS unreachable**, and with `port-control auto` and no critical-auth fallback every access port fails closed | [ ] | `products.ts` (campus port naming/`uplinkStart`) + `configgen.ts` `iosxeCampusConfig` |
| Z5 | Remaining per-vendor items — **NX-OS**: `advertise-pip`/`advertise virtual-rmac` required with a PIP+VIP vPC VTEP + L3VNI (M-2); no `vrf TENANT-A` block under `router bgp` so no type-5 tenant routes (M-3, reverse-parity vs Arista Y4); vPC pair members have different ASNs + no peer-link iBGP (M-6, reverse-parity vs Arista Y4); no `network-qos` jumbo policy so peer-link/host L2 paths stay MTU 1500 (M-5); multihop BFD inert (M-4). **Arista**: mgmt services (tacacs/ntp/syslog/snmp) left in the default VRF while Management1 is in `vrf MGMT` → all non-functional (A3-3); leaf mgmt plane is a subset of the spine's (A3-4); loopbacks re-advertised into BGP create a recursive next-hop on EOS (A3-5); 7800R3 is modular so `Ethernet<slot>/<port>` naming (A3-6). **Juniper**: OOB default route sits in `inet.0` — a data-plane default out the mgmt port; needs `management-instance` (J3-4); no ESI-LAG for dual-homed servers (J3-3); SRX node-1 FPC offset likely `xe-8/0/x` not `xe-7/0/x` (J3-8). **Cumulus**: apply header says `nv config replace` but the artifact is `nv set` lines (must be sourced) (N3-3); no host-side RoCE/NIC manifest — lossless is a host+network contract (N3-4); mgmt by DHCP vs static everywhere else (N3-5). **Both**: vPC/MLAG `pairId` derives from the GLOBAL device index, so an odd preceding-device count splits a pair across two pairIds → mismatched domain/VTEP/peer-link/ASN (M-7/A3-7, latent but fabric-fatal — needs an e2e invariant) | [ ] | `configgen.ts` per-vendor + `haPairInfo` pairId derivation |
| Z6 | Validator V-08 scans **comment** lines — `gpu-nvidia` warns "18 devices have NVE/VXLAN but no EVPN" when there is no VXLAN anywhere; all 18 hits are the comment `# … (jumbo MTU for RoCE/VXLAN payloads)`. Same class as the M9 `extractBgpNeighborIPs` fix: apply the `isCommentLine()` skip to the VXLAN/NVE detector **and audit the other 13 checks for the same flaw** | [ ] | `config-validator.ts` |
| Z7 | Address-scheme scale ceilings (same class as the `alphaLabel` overflow fixed in E3): fabric `10.99.<leaf>.<16·spine+n>` breaks past **254 leaves / 16 spines**; `10.255.2.<globalIdx+1>` and campus `10.255.99.<idx+1>` break past ~250 devices; `10.254.0.<pairId>` at 254 pairs. Beyond those the generator emits invalid IPs **silently**. Needs wider allocation + an e2e invariant asserting every emitted octet ≤ 255 at extreme scale | [ ] | `configgen.ts` address schemes + `e2e-journey.test.ts` |
