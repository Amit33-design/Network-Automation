...
/**
 * ArubaOS-CX management plane, shared by every Aruba role (AM5). The old
 * generator pinned TACACS to `vrf mgmt` but left SNMP, syslog and NTP in the
 * default VRF — which has no route to the OOB network — and toggled the mgmt
 * interface with `shutdown` followed by an invalid `no interface mgmt shutdown`.
 */
function arubaMgmtBlock(hostname: string): string {
...
!
ip dns server-address <CHANGE-ME-dns-ip> vrf mgmt
!
...
lldp enable
!
! ── SSH / AAA (every service in vrf mgmt) ─────────────────────────────────────
ssh server vrf mgmt
...
!
logging <CHANGE-ME-syslog-ip> vrf mgmt severity informational
!`
}

/**
 * ArubaOS-CX spine / leaf (AM5). The Aruba DC fabric was still in the state
 * Z8 fixed for Nokia, Dell and Extreme: `router bgp <CHANGE-ME-asn>` and one
 * `<CHANGE-ME-loopback-ip>` on every device (so eBGP could not form even once
 * filled in), a single placeholder peer, no fabric links, no BFD and no jumbo
 * MTU. Worse, the leaves did not exist as leaves: the BOM placed the CX 6400
 * in its catalogue role, `distribution`, so every leaf-keyed feature (fabric
 * links, VTEP, border-leaf handoff) skipped them. This is the standard
 * treatment the other fabric vendors get — including the AM6 underlay.
 */
function arubaFabricConfig(dev: BOMDevice, idx: number, allDevices: BOMDevice[] = []): string {
  idx = roleIndex(dev, allDevices, idx)
  const isSpine = dev.subLayer === 'spine'
  const asn = isSpine ? 65000 : 65000 + Math.floor(idx / 2) + 1
  const lo0 = isSpine
...
!`).join('\n')
  ...
  // Underlay: one IPv4 session per fabric /31 (AM6). Overlay: EVPN between
  // loopbacks — every leaf on a spine, every LINKED spine on a leaf (Z1/M-1).
  const underlay = links.map(l => {
    const ras = isSpine ? 65000 + Math.floor(l.peerIdx / 2) + 1 : 65000
    return `    neighbor ${l.peerIp} remote-as ${ras}
    neighbor ${l.peerIp} description UNDERLAY: ${l.peerHostname}
    neighbor ${l.peerIp} fall-over bfd`
  })
  const linkedSpines = new Set(links.map(l => l.peerHostname))
  const overlayPeers = isSpine
    ? allDevices.filter(d => d.subLayer === 'leaf').map((d, i) => ({ ip: roleIp('10.255.2.1', RoleSlot.LeafLoopback, i), ras: 65000 + Math.floor(i / 2) + 1, name: d.hostname }))
    : allDevices.filter(d => d.subLayer === 'spine').map((d, i) => ({ d, i }))
      .filter(x => linkedSpines.size === 0 || linkedSpines.has(x.d.hostname))
      .map(x => ({ ip: roleIp('10.255.1.1', RoleSlot.SpineLoopback, x.i), ras: 65000, name: x.d.hostname }))
  const overlay = overlayPeers.map(p => `    neighbor ${p.ip} remote-as ${p.ras}
    neighbor ${p.ip} description OVERLAY: ${p.name}
    neighbor ${p.ip} update-source loopback 0
    neighbor ${p.ip} ebgp-multihop 2
    neighbor ${p.ip} fall-over bfd`)
  const ipv4Activate = links.map(l => `        neighbor ${l.peerIp} activate`).join('\n')
  const evpnActivate = overlayPeers.map(p => `        neighbor ${p.ip} activate
        neighbor ${p.ip} send-community extended${isSpine ? `
  ...
  return `! ═══════════════════════════════════════════════════════════════
! Device : ${dev.hostname}
! Role   : ${isSpine ? 'DC Spine' : 'DC Leaf (VTEP)'}
! Model  : ${dev.model}
! OS     : ArubaOS-CX 10.x
! Generated by NetDesign AI — replace <CHANGE-ME-*> before deploying.
! ═══════════════════════════════════════════════════════════════

${arubaMgmtBlock(dev.hostname)}
bfd
!
interface loopback 0
    ip address ${lo0}/32
!
! ── FABRIC LINKS (topology-driven from BOM port-math) ───────────────────────
${fabricIfaces || '! No fabric peers in this design'}
${isSpine ? '' : `! ── TENANT / VXLAN (leaf only — the spine is not a VTEP) ────────────────────
vlan 10
    name SERVERS
!
interface vxlan 1
...
router bgp ${asn}
    ...
    maximum-paths 64
${[...underlay, ...overlay].join('\n') || '    ! No fabric peers in this design'}
    address-family ipv4 unicast
${ipv4Activate || '        ! No underlay peers'}
        network ${lo0}/32
    exit-address-family
    address-family l2vpn evpn
${evpnActivate || '        ! No overlay peers'}
    exit-address-family
!
`
}
...
/**
 * ArubaOS-CX campus distribution / access (AM5). Distribution used to share
 * the fabric's BGP/EVPN placeholder block — a campus distribution pair runs an
...
!`
    return `! ═══════════════════════════════════════════════════════════════
! Device : ${dev.hostname}
! Role   : Campus Distribution (VRRP pair with ${peerHostname})
! Model  : ${dev.model}
! OS     : ArubaOS-CX 10.x
! Generated by NetDesign AI — replace <CHANGE-ME-*> before deploying.
! ═══════════════════════════════════════════════════════════════
...
${vlans}
spanning-tree
spanning-tree priority ${isPrimary ? 0 : 1}
!
interface loopback 0
    ip address ${lo0}/32
    ip ospf 1 area 0.0.0.0
...
interface 1/1/1-1/1/${dev.ports || 48}
    ...
    spanning-tree port-type admin-edge
!
! Split uplinks, one to each distribution switch: the pair is not a VSX
! cluster, so a single LACP bundle across both would suspend a member (Y3).
interface 1/1/${upStart}
    no shutdown
