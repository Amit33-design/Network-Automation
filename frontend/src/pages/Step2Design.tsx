...
export function Step2Design() {
  ...
  const cabling = useMemo(
    () => buildCabling(generatedDevices, linkDistances),
    [generatedDevices, linkDistances]
  )

  const optics = useMemo(
    () => buildOptics(generatedDevices, linkDistances),
    [generatedDevices, linkDistances]
  )
  ...
  function exportCabling() {
    const header = ['From Layer', 'To Layer', 'Cable Type', 'Speed', 'Length (m)', 'Qty', 'Unit $', 'Total $']
    const dataRows = (cabling as CableLink[]).map(c =>
      [c.fromLayer, c.toLayer, c.cableType, c.speed, c.lengthM, c.quantity, c.pricePerUnit, c.totalPrice].join(',')
    )
    downloadCSV('cabling.csv', [header.join(','), ...dataRows].join('\n'))
  }

  function exportOptics() {
    const header = ['Link Group', 'Form Factor', 'Speed', 'Reach', 'Part Number', 'Qty', 'Unit $', 'Total $']
    const dataRows = (optics as OpticsEntry[]).map(o =>
      [o.linkGroup, o.formFactor, o.speed, o.reach, o.partNumber, o.quantity, o.priceUSD, o.totalPrice].join(',')
    )
    downloadCSV('optics.csv', [header.join(','), ...dataRows].join('\n'))
  }
              ...
              <tbody>
                {(cabling as CableLink[]).map((c, i) => (
                  <tr key={c.id} className={`border-b border-white/5 hover:bg-white/5 ${i % 2 === 0 ? '' : 'bg-white/[0.02]'}`}>
                    <td className="px-4 py-3 text-gray-300"><code className="text-xs text-blue-400">{c.fromLayer}</code></td>
                    <td className="px-4 py-3 text-gray-300"><code className="text-xs text-blue-400">{c.toLayer}</code></td>
        ...
        <div className="overflow-x-auto rounded-xl border border-white/10">
          {optics.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10 bg-white/5">
                  {['Link Group', 'Form Factor', 'Speed', 'Reach', 'Part Number', 'Qty', 'Unit $', 'Total $'].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">{h}</th>
                  ))}
                </tr>
              </thead>
