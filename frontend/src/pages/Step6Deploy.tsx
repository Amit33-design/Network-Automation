...
// Human-readable labels for the 20 supported symptom keys (drives the <select>).
// Grouped by category for the optgroup display.
const TROUBLESHOOT_SYMPTOMS: Array<{ key: string; label: string; group: string }> = [
  // Routing
  { key: 'bgp_down',        label: 'BGP session down',                group: 'Routing' },
  { key: 'ospf_adjacency',  label: 'OSPF adjacency stuck',            group: 'Routing' },
  { key: 'routing_loop',    label: 'Routing loop / TTL exceeded',     group: 'Routing' },
  { key: 'isis_adjacency',  label: 'IS-IS adjacency failure',         group: 'Routing' },
  // Overlay
  { key: 'vxlan_evpn',      label: 'VXLAN / EVPN reachability',       group: 'Overlay' },
  // Physical / L2
  { key: 'interface_flap',  label: 'Interface flapping',              group: 'Physical / L2' },
  { key: 'stp_loop',        label: 'STP loop / root bridge issue',    group: 'Physical / L2' },
  { key: 'mac_flap',        label: 'MAC address flapping',            group: 'Physical / L2' },
  // Performance
  { key: 'high_latency',    label: 'High latency',                    group: 'Performance' },
  { key: 'packet_loss',     label: 'Packet loss',                     group: 'Performance' },
  { key: 'mtu_blackhole',   label: 'MTU / path-MTU black hole',       group: 'Performance' },
  // Device Health
  { key: 'high_cpu',        label: 'High CPU',                        group: 'Device Health' },
  { key: 'memory_exhaustion', label: 'Memory exhaustion / leak',      group: 'Device Health' },
  { key: 'hardware_failure', label: 'Hardware failure (PSU/fan/optic)', group: 'Device Health' },
  // Services
  { key: 'dhcp_failure',    label: 'DHCP failure (relay/pool/snooping)', group: 'Services' },
  { key: 'aaa_auth_failure', label: 'AAA / RADIUS / TACACS+ failure', group: 'Services' },
  { key: 'ntp_sync',        label: 'NTP synchronization failure',     group: 'Services' },
  // Redundancy
  { key: 'hsrp_vrrp',       label: 'HSRP / VRRP / FHRP failure',     group: 'Redundancy' },
  { key: 'vpc_mlag',        label: 'vPC / MLAG peer failure',         group: 'Redundancy' },
  // GPU Fabric
  { key: 'pfc_rocev2',      label: 'PFC / RoCEv2 (GPU fabric)',       group: 'GPU Fabric' },
  // Spine-Leaf Fabric (Cisco + Juniper fabric workflow)
  { key: 'loopback_reachability', label: 'Loopback not reachable (underlay)', group: 'Spine-Leaf Fabric' },
  { key: 'ecmp_inconsistency',    label: 'ECMP path inconsistency / polarization', group: 'Spine-Leaf Fabric' },
  { key: 'border_leaf',           label: 'Border-leaf (external/DCI routes)', group: 'Spine-Leaf Fabric' },
  { key: 'services_leaf',         label: 'Services-leaf (VRF/VLAN/service path)', group: 'Spine-Leaf Fabric' },
]
...
const TROUBLESHOOT_PLAYBOOKS: Record<string, TPlaybook> = {
  ...
  // ── Spine-Leaf Fabric workflow (Cisco + Juniper) ────────────────────────────

  loopback_reachability: {
    category: 'Spine-Leaf Fabric',
    summary: 'A leaf/spine loopback (BGP/VTEP source) is not reachable across the underlay. Confirm the loopback is configured, advertised into the underlay IGP/BGP, and reachable from the remote loopback with the correct source.',
    steps: (p) => [
      { order: 1, description: 'Confirm the local loopback is up and addressed',
        command: tcmd(p, { nxos: 'show ip interface brief | include Loopback', iosxe: 'show ip interface brief | include Loopback', eos: 'show ip interface brief | include Loopback', junos: 'show interfaces terse | match lo0' }),
        look_for: 'Loopback0/lo0 up/up with the expected /32; a down or missing loopback breaks BGP/VTEP sourcing' },
      { order: 2, description: 'Verify the remote loopback is in the route table (advertised in underlay)',
        command: tcmd(p, { nxos: 'show ip route <remote-loopback>', iosxe: 'show ip route <remote-loopback>', eos: 'show ip route <remote-loopback>', junos: 'show route <remote-loopback>' }),
        look_for: 'A /32 via the underlay (IS-IS/OSPF/eBGP); missing = not advertised or filtered' },
      { order: 3, description: 'Ping the remote loopback sourced from the local loopback',
        command: tcmd(p, { nxos: 'ping <remote-loopback> source <local-loopback>', iosxe: 'ping <remote-loopback> source <local-loopback>', eos: 'ping <remote-loopback> source <local-loopback>', junos: 'ping <remote-loopback> source <local-loopback>' }),
        look_for: 'Loss/unreachable — the control-plane/VTEP source cannot reach the peer' },
      { order: 4, description: 'Trace the path to localize where the loopback route is lost',
        command: tcmd(p, { nxos: 'traceroute <remote-loopback> source <local-loopback>', iosxe: 'traceroute <remote-loopback> source <local-loopback>', eos: 'traceroute <remote-loopback> source <local-loopback>', junos: 'traceroute <remote-loopback> source <local-loopback>' }),
        look_for: 'The hop where the trace stops — that node is missing/filtering the loopback prefix' },
    ],
    causes: [
      { cause: 'Loopback not advertised into the underlay (missing network/redistribute/export policy)', confidence: 0.8,
        indicators: ['Remote /32 absent from RIB', 'Local loopback fine but peers cannot reach it', 'IGP export policy omits loopbacks'] },
      { cause: 'Route filtering / prefix-list dropping the loopback range', confidence: 0.6,
        indicators: ['Loopback in IGP database but not RIB', 'Inbound prefix-list/route-map denies the /32', 'Only some peers affected'] },
      { cause: 'Wrong source interface for BGP/VTEP (update-source / source-interface)', confidence: 0.5,
        indicators: ['Ping fails only when sourced from loopback', 'BGP uses wrong update-source', 'VTEP source-interface misconfigured'] },
    ],
    remediation: [
  ...
  ecmp_inconsistency: {
    category: 'Spine-Leaf Fabric',
    summary: 'Leaf-to-leaf traffic is not load-balancing across all spines (missing ECMP next-hops or polarization). Confirm all equal-cost paths are installed and the hardware load-balance hashing is healthy.',
    steps: (p) => [
      { order: 1, description: 'Check the route has multiple equal-cost next-hops (one per spine)',
        command: tcmd(p, { nxos: 'show ip route <destination>', iosxe: 'show ip route <destination>', eos: 'show ip route <destination>', junos: 'show route <destination>' }),
        look_for: 'Fewer next-hops than spines = missing ECMP path (max-paths or a down underlay adjacency)' },
      { order: 2, description: 'Confirm the FIB/forwarding table installed all paths',
        command: tcmd(p, { nxos: 'show ip cef <destination>', iosxe: 'show ip cef <destination>', eos: 'show ip route <destination> detail', junos: 'show route forwarding-table destination <destination>' }),
        look_for: 'RIB shows N paths but FIB installs fewer — a hardware/max-paths limit' },
      { order: 3, description: 'Verify the exact path a flow will take (hashing/polarization)',
        command: tcmd(p, { nxos: 'show ip cef exact-route <src-ip> <dst-ip>', iosxe: 'show ip cef exact-route <src-ip> <dst-ip>', eos: 'show route exact-route <src-ip> <dst-ip>', junos: 'show route extensive <destination>' }),
        look_for: 'All flows resolving to one spine = polarization (identical hash seed across tiers)' },
      { order: 4, description: 'Check port-channel / LAG member load-balance hashing',
        command: tcmd(p, { nxos: 'show port-channel load-balance', iosxe: 'show etherchannel load-balance', eos: 'show port-channel load-balance', junos: 'show chassis forwarding-options' }),
    ...
    causes: [
      { cause: 'maximum-paths too low — not all spine paths installed', confidence: 0.75,
        indicators: ['RIB has fewer next-hops than spines', 'maximum-paths 1/2 on a wider fabric', 'Throughput capped to one uplink'] },
      { cause: 'ECMP polarization (same hash across leaf→spine→leaf)', confidence: 0.6,
        indicators: ['All flows take one spine', 'Uneven spine utilization', 'Default hash seed everywhere'] },
      { cause: 'A spine uplink/underlay adjacency is down', confidence: 0.55,
        indicators: ['One next-hop missing', 'IGP/BGP adjacency down to a spine', 'Recent link flap'] },
    ],
    remediation: [
  ...
  border_leaf: {
    ...
    steps: (p) => [
      { order: 1, description: 'Check the external/default route is present and learned correctly',
        command: tcmd(p, { nxos: 'show ip route <prefix>', iosxe: 'show ip route <prefix>', eos: 'show ip route <prefix>', junos: 'show route <prefix>' }),
        look_for: 'Missing external prefix / no default — import policy or peering issue' },
      { order: 2, description: 'Verify the external BGP peering is up and exchanging routes',
        command: tcmd(p, { nxos: 'show bgp ipv4 unicast summary', iosxe: 'show ip bgp summary', eos: 'show ip bgp summary', junos: 'show bgp summary' }),
        look_for: 'External/DCI peer not Established, or PfxRcd 0 — no external routes coming in' },
      { order: 3, description: 'Confirm which prefixes are advertised to the external/DCI peer',
        command: tcmd(p, { nxos: 'show ip bgp neighbors <peer> advertised-routes', iosxe: 'show ip bgp neighbors <peer> advertised-routes', eos: 'show ip bgp neighbors <peer> advertised-routes', junos: 'show route advertising-protocol bgp <peer>' }),
        look_for: 'Fabric/tenant prefixes missing from the advertisement — export policy gap (no return path)' },
      { order: 4, description: 'Inspect the route-map / policy controlling import/export + filtering',
        command: tcmd(p, { nxos: 'show route-map', iosxe: 'show route-map', eos: 'show route-map', junos: 'show configuration policy-options' }),
    ...
    causes: [
      ...
      { cause: 'Export policy omits fabric prefixes — broken return path', confidence: 0.65,
        indicators: ['advertised-routes missing tenant prefixes', 'One-way reachability', 'Asymmetric/return-path drop'] },
      { cause: 'External BGP session down / prefix limit hit', confidence: 0.5,
        indicators: ['DCI/WAN peer not Established', 'PfxRcd 0 or maxed', 'Notification: max-prefix'] },
    ],
    remediation: [
  ...
  services_leaf: {
    ...
    steps: (p) => [
      { order: 1, description: 'Check the service-facing interface and VLAN are up/associated',
        command: tcmd(p, { nxos: 'show ip interface brief', iosxe: 'show ip interface brief', eos: 'show ip interface brief', junos: 'show interfaces terse' }),
        look_for: 'Service interface down, or not in the expected VLAN/VRF' },
      { order: 2, description: 'Confirm VLAN ↔ VRF association',
        command: tcmd(p, { nxos: 'show vrf', iosxe: 'show vrf', eos: 'show vrf', junos: 'show route instance' }),
        look_for: 'SVI/interface not bound to the service VRF, or VLAN missing from the VLAN db' },
      { order: 3, description: 'Verify routing to the service subnet inside the VRF',
        command: tcmd(p, { nxos: 'show ip route vrf <vrf-name>', iosxe: 'show ip route vrf <vrf-name>', eos: 'show ip route vrf <vrf-name>', junos: 'show route table <vrf-name>.inet.0' }),
        look_for: 'No route to the service/firewall/LB subnet within the tenant VRF' },
      { order: 4, description: 'Test reachability to the service IP from within the VRF',
        command: tcmd(p, { nxos: 'ping vrf <vrf-name> <service-ip>', iosxe: 'ping vrf <vrf-name> <service-ip>', eos: 'ping vrf <vrf-name> <service-ip>', junos: 'ping routing-instance <vrf-name> <service-ip>' }),
    ...
    causes: [
      { cause: 'VLAN/VRF association wrong on the service interface', confidence: 0.75,
        indicators: ['Service SVI in wrong/global VRF', 'VLAN not in db', 'Route present in global not tenant VRF'] },
      { cause: 'No route to the service subnet inside the tenant VRF', confidence: 0.6,
        indicators: ['show ip route vrf missing the subnet', 'Service unreachable only in-VRF', 'Missing import RT for the service'] },
      { cause: 'Asymmetric routing breaking stateful firewall/NAT/LB', confidence: 0.55,
        indicators: ['One-direction works', 'Firewall drops out-of-state', 'Different forward vs return path'] },
    ],
    remediation: [
      ...
      'Make the forward and return paths symmetric (or enable the firewall asymmetric/round-robin handling)',
      'Re-test with an in-VRF ping/traceroute to the service IP',
    ],
  },

  // ── TAC KB expansion (12 new categories) ────────────────────────────────────

  stp_loop: {
    category: 'L2 / Spanning Tree',
    summary: 'A spanning-tree loop or root-bridge election issue is causing broadcast storms, MAC flapping, or ports stuck in blocking. Verify root-bridge priority, BPDU guard, and loop-guard.',
    steps: (p) => [
      { order: 1, description: 'Check the STP root bridge for each VLAN',
        command: tcmd(p, { nxos: 'show spanning-tree root', iosxe: 'show spanning-tree root', eos: 'show spanning-tree root all', junos: 'show spanning-tree bridge' }),
        look_for: 'Unexpected root bridge — a rogue switch with lower priority hijacking root' },
      { order: 2, description: 'Look for ports in err-disabled / BPDU-guard violation',
        command: tcmd(p, { nxos: 'show interface status err-disabled', iosxe: 'show interfaces status err-disabled', eos: 'show errdisabled', junos: 'show ethernet-switching interface | match block' }),
        look_for: 'Ports shut by BPDU guard — an upstream switch is sending BPDUs into an access port' },
      { order: 3, description: 'Verify loop guard / root guard status',
        command: tcmd(p, { nxos: 'show spanning-tree detail | include guard', iosxe: 'show spanning-tree detail | include guard', eos: 'show spanning-tree detail | include guard', junos: 'show spanning-tree interface detail' }),
        look_for: 'Loop-guard or root-guard triggered — indicates a unidirectional link or topology misconfiguration' },
      { order: 4, description: 'Check for broadcast storm / high CPU from L2 flooding',
        command: tcmd(p, { nxos: 'show interface counters storm-control', iosxe: 'show storm-control broadcast', eos: 'show storm-control', junos: 'show ethernet-switching flood-statistics' }),
        look_for: 'Storm-control drops rising — active broadcast storm; locate the source port' },
      { order: 5, description: 'Review STP topology change (TC) history',
        command: tcmd(p, { nxos: 'show spanning-tree detail | include topology', iosxe: 'show spanning-tree detail | include topology', eos: 'show spanning-tree topology-change detail', junos: 'show spanning-tree bridge detail' }),
        look_for: 'Excessive topology changes — an unstable port is triggering continuous MAC re-learning' },
    ],
    causes: [
      { cause: 'Rogue root bridge (lower priority device elected root)', confidence: 0.8,
        indicators: ['Unexpected root bridge ID', 'Traffic taking suboptimal path', 'Root on an access switch'] },
      { cause: 'Unidirectional link causing loop-guard trigger', confidence: 0.65,
        indicators: ['Loop-guard inconsistent state', 'Fiber rx ok / tx dark', 'One-way BPDUs'] },
      { cause: 'BPDU guard violation on an access port', confidence: 0.55,
        indicators: ['Port err-disabled', 'BPDU received on portfast interface', 'Rogue switch plugged in'] },
      { cause: 'Missing or inconsistent STP configuration across VLANs', confidence: 0.4,
        indicators: ['Different root per VLAN', 'No root-guard on designated ports', 'PVST+/RPVST mismatch'] },
    ],
    remediation: [
  ...
  dhcp_failure: {
    category: 'Services',
    summary: 'DHCP clients are not obtaining IP addresses. The issue may be at the DHCP server, relay agent, or client VLAN. Check relay config, pool exhaustion, snooping trust, and Option 82.',
    steps: (p) => [
      { order: 1, description: 'Verify the DHCP relay (ip helper-address) configuration',
        command: tcmd(p, { nxos: 'show ip interface <vlan-intf> | include helper', iosxe: 'show ip interface <vlan-intf> | include helper', eos: 'show ip interface <vlan-intf> | include helper', junos: 'show forwarding-options dhcp-relay interface <vlan-intf>' }),
        look_for: 'Missing or incorrect ip helper-address pointing to the DHCP server' },
      { order: 2, description: 'Check DHCP snooping status and trust configuration',
        command: tcmd(p, { nxos: 'show ip dhcp snooping', iosxe: 'show ip dhcp snooping', eos: 'show ip dhcp snooping', junos: 'show dhcp-security binding' }),
        look_for: 'Snooping enabled but uplink/server port not trusted — DHCP offers get dropped' },
      { order: 3, description: 'Verify DHCP server pool and lease availability',
        command: tcmd(p, { nxos: 'show ip dhcp relay statistics', iosxe: 'show ip dhcp pool', eos: 'show ip dhcp relay counters', junos: 'show system services dhcp pool' }),
        look_for: 'Pool exhaustion (0 addresses available) or relay drops/no-response counters' },
      { order: 4, description: 'Capture or count DHCP discover/offer/request/ack',
        command: tcmd(p, { nxos: 'show ip dhcp relay statistics', iosxe: 'show ip dhcp server statistics', eos: 'show ip dhcp relay counters', junos: 'show dhcp-security statistics' }),
        look_for: 'Discovers sent but no Offers received — relay not forwarding, or server unreachable' },
    ],
    causes: [
      { cause: 'Missing or incorrect ip helper-address / relay configuration', confidence: 0.8,
        indicators: ['No helper-address on SVI', 'Wrong server IP', 'Relay not relaying Discovers'] },
      { cause: 'DHCP snooping dropping server replies (uplink not trusted)', confidence: 0.7,
        indicators: ['Snooping enabled, uplink untrusted', 'Offer/Ack drops in snooping stats', 'Works when snooping disabled'] },
      { cause: 'DHCP pool exhaustion (no available addresses)', confidence: 0.55,
        indicators: ['Pool shows 0 free', 'Stale leases not reclaimed', 'Scope too small for clients'] },
      { cause: 'Option 82 / relay-information policy mismatch', confidence: 0.4,
        indicators: ['Server rejects relayed packets', 'Option 82 insert vs drop conflict', 'Works from server VLAN directly'] },
    ],
    remediation: [
  ...
  mtu_blackhole: {
    category: 'Performance',
    summary: 'Large packets (>1500 or >9000) are silently dropped mid-path due to an MTU mismatch. TCP connections stall on large transfers while small packets/pings succeed.',
    steps: (p) => [
      { order: 1, description: 'Test path MTU with DF-bit set at incremental sizes',
        command: tcmd(p, { nxos: 'ping <dest> df-bit packet-size 1500', iosxe: 'ping <dest> df-bit size 1500', eos: 'ping <dest> df-bit size 1500', junos: 'ping <dest> do-not-fragment size 1500' }),
        look_for: 'Success at 1500 but failure at 9000 (or vice versa) pinpoints the MTU boundary' },
      { order: 2, description: 'Check interface MTU on each hop in the path',
        command: tcmd(p, { nxos: 'show interface <intf> | include MTU', iosxe: 'show interfaces <intf> | include MTU', eos: 'show interfaces <intf> | include MTU', junos: 'show interfaces <intf> | match mtu' }),
        look_for: 'A link with MTU 1500 in a jumbo-frame path, or vice versa' },
      { order: 3, description: 'Look for ICMP "fragmentation needed" responses',
        command: tcmd(p, { nxos: 'show ip icmp statistics | include frag', iosxe: 'show ip icmp statistics | include frag', eos: 'show ip icmp counters | include frag', junos: 'show system statistics icmp | match frag' }),
        look_for: 'Outbound "frag needed but DF set" — the device generating the ICMP is the MTU bottleneck' },
      { order: 4, description: 'Verify TCP MSS clamping if configured',
        command: tcmd(p, { nxos: 'show running-config | include tcp.*mss', iosxe: 'show running-config | include ip tcp adjust-mss', eos: 'show running-config | include mss', junos: 'show configuration class-of-service | match tcp-mss' }),
        look_for: 'TCP MSS adjust may be a workaround in place — or may need to be added' },
    ],
    causes: [
      { cause: 'Inconsistent MTU on one link in the path (1500 vs 9216)', confidence: 0.85,
        indicators: ['DF-bit ping fails at certain sizes', 'TCP stalls on large transfers', 'Small pings succeed'] },
      { cause: 'Firewall / middle-box dropping ICMP "frag needed" messages', confidence: 0.6,
        indicators: ['No ICMP unreachable returned', 'PMTUD fails silently', 'Works when DF-bit cleared'] },
      { cause: 'GRE/VXLAN/IPsec overhead reducing effective MTU', confidence: 0.5,
        indicators: ['Tunnel interface in path', 'Drops at exactly MTU − overhead', 'Inner packet too large'] },
    ],
    remediation: [
  ...
  aaa_auth_failure: {
    category: 'Services',
    summary: 'AAA/RADIUS/TACACS+ authentication is failing. Users cannot log in, or commands are being denied. Verify server reachability, shared secret, and the AAA method list.',
    steps: (p) => [
      { order: 1, description: 'Test reachability to the AAA server',
        command: tcmd(p, { nxos: 'ping <aaa-server-ip>', iosxe: 'ping <aaa-server-ip>', eos: 'ping <aaa-server-ip>', junos: 'ping <aaa-server-ip>' }),
        look_for: 'Packet loss or unreachable — server or routing issue' },
      { order: 2, description: 'Check AAA server configuration and status',
        command: tcmd(p, { nxos: 'show aaa authentication', iosxe: 'show aaa servers', eos: 'show aaa', junos: 'show system aaa' }),
        look_for: 'Server marked dead, high timeout count, or wrong port/protocol configured' },
      { order: 3, description: 'Verify RADIUS/TACACS+ statistics for success vs failure',
        command: tcmd(p, { nxos: 'show radius-server statistics', iosxe: 'show aaa servers', eos: 'show radius counters', junos: 'show network-access aaa statistics' }),
        look_for: 'High reject/timeout count — shared-secret mismatch or server-side policy denying' },
      { order: 4, description: 'Check the fallback method list (local auth as backup)',
        command: tcmd(p, { nxos: 'show aaa authentication', iosxe: 'show running-config | section aaa', eos: 'show running-config | section aaa', junos: 'show configuration system authentication-order' }),
        look_for: '"local" not in the method list — if RADIUS is down, no login at all' },
    ],
    causes: [
      { cause: 'RADIUS/TACACS+ server unreachable (routing or firewall)', confidence: 0.8,
        indicators: ['Ping fails', 'Timeout counters rising', 'Server marked dead'] },
      { cause: 'Shared secret mismatch between device and server', confidence: 0.7,
        indicators: ['Access-Reject immediately', 'Wireshark shows bad-authenticator', '"Invalid shared secret" in server logs'] },
      { cause: 'AAA method list missing "local" fallback', confidence: 0.55,
        indicators: ['Cannot login when server is down', 'Only remote method in aaa authentication', 'Console locked out'] },
      { cause: 'Server-side policy denying the user/group', confidence: 0.4,
        indicators: ['Access-Reject with valid credentials', 'Works with a different user', 'Policy/ACL on RADIUS server'] },
    ],
    remediation: [
      'Restore L3 reachability to the AAA server; check any firewall rules on UDP 1812/1813 or TCP 49',
      'Align the shared secret on both the device and the server (re-key if needed)',
      'Always include "local" as the last method in the aaa authentication method list',
  ...
  hsrp_vrrp: {
    category: 'Redundancy',
    summary: 'The HSRP/VRRP virtual gateway is down or both routers think they are active (dual-active). Check priority, preemption, tracking, and the keepalive path.',
    steps: (p) => [
      { order: 1, description: 'Check HSRP/VRRP group state on both routers',
        command: tcmd(p, { nxos: 'show hsrp brief', iosxe: 'show standby brief', eos: 'show vrrp', junos: 'show vrrp summary' }),
        look_for: 'Both routers Active/Master (dual-active) or no Active at all (VIP unreachable)' },
      { order: 2, description: 'Verify priority and preemption configuration',
        command: tcmd(p, { nxos: 'show hsrp detail', iosxe: 'show standby detail', eos: 'show vrrp detail', junos: 'show vrrp detail' }),
        look_for: 'Priority mismatch, preempt disabled when it should be enabled, or track objects decrementing priority' },
      { order: 3, description: 'Check the keepalive / hello path between peers',
        command: tcmd(p, { nxos: 'ping <peer-ip>', iosxe: 'ping <peer-ip>', eos: 'ping <peer-ip>', junos: 'ping <peer-ip>' }),
        look_for: 'Loss between peers — if hellos do not cross, both go Active (split-brain)' },
      { order: 4, description: 'Look for tracked interface/object state changes',
        command: tcmd(p, { nxos: 'show track', iosxe: 'show track', eos: 'show track', junos: 'show vrrp track' }),
        look_for: 'A tracked interface going down decrementing priority below the peer — unexpected failover' },
    ],
    causes: [
      { cause: 'Dual-active / split-brain (keepalives not crossing)', confidence: 0.85,
        indicators: ['Both routers Active/Master', 'Duplicate VIP ARP', 'L2 path between peers broken'] },
      { cause: 'Track object decrementing priority causing unexpected failover', confidence: 0.65,
        indicators: ['Tracked interface down', 'Priority decremented', 'Standby became Active after track change'] },
      { cause: 'Preemption misconfiguration (failback not happening)', confidence: 0.5,
        indicators: ['Original primary stays Standby after recovery', 'preempt not configured', 'Lower-priority stays Active'] },
      { cause: 'Timer mismatch (hello/hold differ between peers)', confidence: 0.35,
        indicators: ['Intermittent VIP unreachable', 'Aggressive timers on one side', 'Flapping Active/Standby'] },
    ],
    remediation: [
      'Restore L2 reachability between HSRP/VRRP peers (fix the shared segment)',
      'Review tracked objects and their decrement values — align with desired failover behavior',
  ...
  mac_flap: {
    ...
    summary: 'A MAC address is being learned on multiple ports, causing the switch to flap the entry back and forth. Typically indicates a loop, a VM migration, or an incorrect VLAN trunk.',
    steps: (p) => [
      { order: 1, description: 'Check syslog for MAC move/flap notifications',
        command: tcmd(p, { nxos: 'show logging | include MAC_MOVE|flap', iosxe: 'show logging | include MAC_MOVE|FLAP', eos: 'show logging | include MAC_MOVE|FLAP', junos: 'show log messages | match "MAC move"' }),
        look_for: 'Repeated MAC move messages between two ports — identifies the flapping MAC and ports' },
      { order: 2, description: 'Verify the MAC address table for the affected address',
        command: tcmd(p, { nxos: 'show mac address-table address <mac>', iosxe: 'show mac address-table address <mac>', eos: 'show mac address-table address <mac>', junos: 'show ethernet-switching table <mac>' }),
        look_for: 'The MAC alternating between two ports — one is the correct path, the other is the loop/mirror' },
      { order: 3, description: 'Check for STP state and VLAN membership on both ports',
        command: tcmd(p, { nxos: 'show spanning-tree interface <intf>', iosxe: 'show spanning-tree interface <intf>', eos: 'show spanning-tree interface <intf>', junos: 'show spanning-tree interface <intf>' }),
        look_for: 'Both ports forwarding in the same VLAN — one should be blocking if STP is working' },
      { order: 4, description: 'Look for SPAN/RSPAN or other mirroring sessions',
        command: tcmd(p, { nxos: 'show monitor session all', iosxe: 'show monitor session all', eos: 'show monitor session', junos: 'show forwarding-options analyzer' }),
        look_for: 'An active SPAN session reflecting traffic can cause MAC flaps on the destination port' },
    ],
    causes: [
      { cause: 'Layer-2 loop (STP not blocking a redundant path)', confidence: 0.8,
        indicators: ['Two ports forwarding in same VLAN', 'Broadcast storm', 'High CPU from flooding'] },
      { cause: 'VM / workload migration (legitimate MAC move)', confidence: 0.55,
        indicators: ['MAC moves once then stabilizes', 'vMotion/live-migration event', 'No loop symptoms'] },
      { cause: 'Incorrect VLAN trunking (VLAN leaking across domains)', confidence: 0.5,
        indicators: ['VLAN present on unexpected trunk', 'MAC seen from an uplink that shouldn\'t carry it'] },
      { cause: 'SPAN/mirror session reflecting traffic', confidence: 0.35,
        indicators: ['Monitor session active', 'Flapping stops when SPAN disabled', 'Destination port shows the MAC'] },
    ],
    remediation: [
      'If a loop: shut the offending port, fix STP (enable BPDU guard, loop guard)',
      'For VM migration: configure MAC-move allow or increase mac-address-table aging for the VLAN',
  ...
  vpc_mlag: {
    category: 'Redundancy',
    summary: 'The vPC (Cisco) or MLAG (Arista) peer link or keepalive is down, causing split-brain or port suspension. Check peer-link, keepalive, domain consistency, and port-channel hashing.',
    steps: (p) => [
      { order: 1, description: 'Check vPC/MLAG domain status and peer reachability',
        command: tcmd(p, { nxos: 'show vpc', iosxe: 'show etherchannel summary', eos: 'show mlag', junos: 'show multi-chassis mc-lag status' }),
        look_for: 'Peer not reachable, peer-link down, or dual-active detection triggered' },
      { order: 2, description: 'Verify the keepalive link status and messages',
        command: tcmd(p, { nxos: 'show vpc peer-keepalive', iosxe: 'show redundancy', eos: 'show mlag detail', junos: 'show multi-chassis mc-lag status' }),
        look_for: 'Keepalive timeout / peer unreachable — triggers secondary to suspend vPC ports' },
      { order: 3, description: 'Check for type-1 consistency violations',
        command: tcmd(p, { nxos: 'show vpc consistency-parameters global', iosxe: 'show etherchannel detail', eos: 'show mlag config-sanity', junos: 'show multi-chassis mc-lag configuration' }),
        look_for: 'STP mode, VLAN list, or MTU mismatch — type-1 violations suspend the vPC/MLAG' },
      { order: 4, description: 'Verify individual member port-channel status',
        command: tcmd(p, { nxos: 'show vpc brief', iosxe: 'show etherchannel summary', eos: 'show mlag interfaces', junos: 'show multi-chassis mc-lag interfaces' }),
        look_for: 'vPC/MLAG port suspended or only active on one side' },
    ],
    causes: [
      { cause: 'Peer-link down (L2 trunk between vPC/MLAG peers failed)', confidence: 0.85,
        indicators: ['Peer-link admin/oper down', 'Orphan ports active', 'Secondary suspends all vPCs'] },
      { cause: 'Keepalive timeout / management path failure', confidence: 0.7,
        indicators: ['Keepalive lost', 'Peer assumed dead', 'Both peers go primary (split-brain)'] },
      { cause: 'Type-1 consistency check failure (STP/VLAN/MTU mismatch)', confidence: 0.6,
        indicators: ['Consistency check failed', 'Specific vPCs suspended', '"Type 1" in show vpc'] },
      { cause: 'Port-channel member mismatch (LACP negotiation issue)', confidence: 0.4,
        indicators: ['LACP not converging', 'Member ports suspended individually', 'Speed/duplex mismatch'] },
    ],
    remediation: [
      ...
      'Resolve type-1 consistency violations (align STP mode, VLAN, MTU on both peers)',
  ...
  ntp_sync: {
    category: 'Services',
    summary: 'NTP is not synchronized — the device clock is drifting. This affects syslog timestamps, certificate validation, DNSSEC, and Kerberos/AAA. Verify NTP server reachability and stratum.',
    steps: (p) => [
      { order: 1, description: 'Check NTP association and synchronization status',
        command: tcmd(p, { nxos: 'show ntp peer-status', iosxe: 'show ntp associations', eos: 'show ntp status', junos: 'show ntp associations' }),
        look_for: 'No "*" (selected source), all peers showing ".INIT." or "reject" — no valid time source' },
      { order: 2, description: 'Verify reachability to the NTP server',
        command: tcmd(p, { nxos: 'ping <ntp-server-ip>', iosxe: 'ping <ntp-server-ip>', eos: 'ping <ntp-server-ip>', junos: 'ping <ntp-server-ip>' }),
        look_for: 'Packet loss or unreachable — routing/firewall issue blocking UDP 123' },
      { order: 3, description: 'Check the NTP source interface configuration',
        command: tcmd(p, { nxos: 'show running-config | include ntp', iosxe: 'show running-config | include ntp', eos: 'show running-config | section ntp', junos: 'show configuration system ntp' }),
        look_for: 'Source interface not in the routing table, or wrong VRF for the NTP server' },
      { order: 4, description: 'Verify the clock offset and stratum',
        command: tcmd(p, { nxos: 'show ntp peer-status', iosxe: 'show ntp status', eos: 'show ntp status', junos: 'show ntp status' }),
        look_for: 'Large offset (>1000 ms) may prevent sync; stratum 16 = unsynchronized' },
    ],
    causes: [
      { cause: 'NTP server unreachable (routing/firewall blocking UDP 123)', confidence: 0.8,
        indicators: ['Ping fails', 'Reach = 0', '.INIT. state', 'ACL dropping UDP 123'] },
      { cause: 'NTP source interface down or in wrong VRF', confidence: 0.6,
        indicators: ['Source interface specified but down', 'NTP server in different VRF', 'Packets never sent'] },
      { cause: 'Clock offset too large for NTP to sync (>1000s)', confidence: 0.5,
        indicators: ['Stratum 16', 'Huge offset in show ntp', 'Device time way off', 'Need manual clock set'] },
      { cause: 'NTP authentication key mismatch', confidence: 0.35,
        indicators: ['Auth enabled, key mismatch', 'Rejected packets in NTP stats', 'Works without auth'] },
    ],
    remediation: [
      ...
      'If offset > 1000s, manually set the clock close to correct time, then NTP will sync',
  ...
  hardware_failure: {
    category: 'Device Health',
    summary: 'A hardware component (PSU, fan, line card, supervisor, or optic) is failing or degraded. Check environmental sensors, module status, and syslog for hardware alarms.',
    steps: (p) => [
      { order: 1, description: 'Check power supply status',
        command: tcmd(p, { nxos: 'show environment power', iosxe: 'show environment power', eos: 'show environment power', junos: 'show chassis power' }),
        look_for: 'PSU failed/absent — risk of single-PSU operation or total power loss' },
      { order: 2, description: 'Verify fan tray status and temperature',
        command: tcmd(p, { nxos: 'show environment fan', iosxe: 'show environment', eos: 'show environment cooling', junos: 'show chassis environment' }),
        look_for: 'Fan failed or temperature critical — device may thermal-shutdown' },
      { order: 3, description: 'Check line card / module operational status',
        command: tcmd(p, { nxos: 'show module', iosxe: 'show module', eos: 'show module', junos: 'show chassis fpc' }),
        look_for: 'Module powered-off/failed/inserted but not online — a linecard failure' },
      { order: 4, description: 'Look for transceiver / optic alarms',
        command: tcmd(p, { nxos: 'show interface transceiver details', iosxe: 'show interfaces transceiver detail', eos: 'show interfaces transceiver', junos: 'show interfaces diagnostics optics' }),
        look_for: 'DOM high/low alarm on Rx/Tx power, temperature, or voltage — failing optic' },
      { order: 5, description: 'Review syslog for hardware error events',
        command: tcmd(p, { nxos: 'show logging | include HARDWARE|POWER|FAN|SENSOR', iosxe: 'show logging | include ENVIRONMENT|POWER|FAN', eos: 'show logging | include hardware|power|fan', junos: 'show log messages | match "CHASSIS|ALARM|FPC"' }),
        look_for: 'Hardware alarm events with timestamps — correlate with the observed issue' },
    ],
    causes: [
      { cause: 'Power supply failure (single PSU operation)', confidence: 0.75,
        indicators: ['PSU status: failed/absent', 'Power alarm in syslog', 'Reduced power budget'] },
      { cause: 'Fan failure causing thermal shutdown risk', confidence: 0.65,
        indicators: ['Fan status: failed', 'Temperature rising', 'Thermal alarm syslog'] },
      { cause: 'Line card / FPC offline or failing', confidence: 0.55,
        indicators: ['Module not online', 'Ports on the module down', 'FPC restarting'] },
      { cause: 'Failing optical transceiver (DOM alarms)', confidence: 0.5,
        indicators: ['Rx/Tx power outside spec', 'Temperature alarm', 'CRC errors on that port'] },
    ],
    remediation: [
  ...
  memory_exhaustion: {
    category: 'Device Health',
    summary: 'Device memory is critically high or exhausted. This can cause process crashes, routing instability, and loss of management access. Identify the top memory consumer.',
    steps: (p) => [
      { order: 1, description: 'Check overall system memory utilization',
        command: tcmd(p, { nxos: 'show system resources', iosxe: 'show processes memory sorted', eos: 'show processes top once', junos: 'show chassis routing-engine' }),
        look_for: 'Memory utilization above 85% — risk of OOM kills and process instability' },
      { order: 2, description: 'Identify the top memory-consuming process',
        command: tcmd(p, { nxos: 'show processes memory | head 20', iosxe: 'show processes memory sorted | head 20', eos: 'show processes top once', junos: 'show system processes extensive | match "PID|Mem"' }),
        look_for: 'A single process consuming disproportionate memory — potential memory leak' },
      { order: 3, description: 'Check the routing table size (BGP RIB scale)',
        command: tcmd(p, { nxos: 'show ip route summary', iosxe: 'show ip route summary', eos: 'show ip route summary', junos: 'show route summary' }),
        look_for: 'Very large route count (full DFZ table without enough memory) can exhaust memory' },
      { order: 4, description: 'Look for memory-related syslog events',
        command: tcmd(p, { nxos: 'show logging | include memory|MEM|OOM', iosxe: 'show logging | include MALLOCFAIL|SYS-2-MALLOCFAIL', eos: 'show logging | include memory|oom', junos: 'show log messages | match "memory|KERNEL"' }),
        look_for: 'MALLOCFAIL / OOM events — process was denied memory allocation' },
    ],
    causes: [
      { cause: 'Memory leak in a process (gradual increase over time)', confidence: 0.75,
        indicators: ['One process memory growing over days', 'Resolves after process restart', 'Known software bug'] },
      { cause: 'Route table exhaustion (full Internet table on undersized platform)', confidence: 0.65,
        indicators: ['500K+ routes in RIB', 'BGP process is top memory consumer', 'Platform has limited RAM'] },
      { cause: 'Configuration bloat (very large ACLs, prefix-lists, or object-groups)', confidence: 0.45,
        indicators: ['Large running-config', 'Config process high memory', 'Thousands of ACL entries'] },
      { cause: 'ARP/MAC/FIB table explosion from broadcast domain too large', confidence: 0.4,
        indicators: ['show arp count very high', 'Flat L2 domain', 'ARP process memory growing'] },
    ],
    remediation: [
      'If a memory leak: restart the affected process, then plan a software upgrade with the fix',
      'For route exhaustion: add ORF/filter to limit received prefixes, or upgrade platform RAM',
      'Reduce configuration size (consolidate ACLs, use object-groups, remove unused entries)',
  ...
  routing_loop: {
    category: 'Routing',
    summary: 'Traffic is looping between hops — packets hit TTL expiry and are dropped. Traceroute shows the same hops repeating. Usually caused by asymmetric static routes or redistribution.',
    steps: (p) => [
      { order: 1, description: 'Trace the path to the destination and look for loops',
        command: tcmd(p, { nxos: 'traceroute <dest>', iosxe: 'traceroute <dest>', eos: 'traceroute <dest>', junos: 'traceroute <dest>' }),
        look_for: 'Hops repeating (A → B → A → B) — the loop point is between these two devices' },
      { order: 2, description: 'Check the routing table on each device in the loop',
        command: tcmd(p, { nxos: 'show ip route <dest>', iosxe: 'show ip route <dest>', eos: 'show ip route <dest>', junos: 'show route <dest>' }),
        look_for: 'Each device points to the other as next-hop — recursive or mutual static routes' },
      { order: 3, description: 'Look for route redistribution or default route injection',
        command: tcmd(p, { nxos: 'show ip route <dest> detail', iosxe: 'show ip route <dest>', eos: 'show ip route <dest> detail', junos: 'show route <dest> detail' }),
        look_for: 'Route learned via redistribution from a routing protocol that should not advertise it' },
      { order: 4, description: 'Check ICMP TTL-exceeded counters',
        command: tcmd(p, { nxos: 'show ip icmp statistics', iosxe: 'show ip icmp statistics', eos: 'show ip icmp counters', junos: 'show system statistics icmp | match "time exceed"' }),
        look_for: 'High TTL-exceeded count — confirms packets are looping and being dropped' },
    ],
    causes: [
      { cause: 'Mutual static/default routes creating a forwarding loop', confidence: 0.8,
        indicators: ['Static route to peer, peer has static back', 'No dynamic routing for the prefix', 'Asymmetric next-hops'] },
      { cause: 'Route redistribution feedback loop', confidence: 0.65,
        indicators: ['Route redistributed into the protocol it came from', 'No route-map filtering redistribution', 'Suboptimal route installed'] },
      { cause: 'Policy-based routing (PBR) overriding the RIB', confidence: 0.45,
        indicators: ['PBR on one device, RIB on the other', 'Packets match PBR and loop back', 'Works when PBR disabled'] },
      { cause: 'Summarization or default-route causing a less-specific match', confidence: 0.4,
        indicators: ['Summary route attracts traffic meant for a more-specific', 'Null0 / discard route missing', 'Asymmetric summarization'] },
    ],
    remediation: [
      'Remove or fix the conflicting static routes; use a dynamic protocol if possible',
      'Add route-map tags and deny on redistribution to prevent feedback loops',
      'If PBR is involved, ensure both directions are consistent or add a PBR deny for return traffic',
      'Add a Null0 / discard route for the summary to prevent routing loops on less-specific matches',
    ],
  },

  isis_adjacency: {
    category: 'Routing',
    summary: 'IS-IS neighbors are not forming or are stuck. Common in DC spine-leaf fabrics. Check system-id, area, level, authentication, and interface IS-IS enablement.',
    steps: (p) => [
      { order: 1, description: 'Check IS-IS adjacency state',
        command: tcmd(p, { nxos: 'show isis adjacency', iosxe: 'show isis neighbors', eos: 'show isis neighbors', junos: 'show isis adjacency' }),
        look_for: 'Missing neighbor or state DOWN — adjacency not forming' },
      { order: 2, description: 'Verify IS-IS is enabled on the interface',
        command: tcmd(p, { nxos: 'show isis interface brief', iosxe: 'show isis interface brief', eos: 'show isis interface brief', junos: 'show isis interface' }),
        look_for: 'Interface not listed — IS-IS not enabled on the link, or wrong IS-IS instance' },
      { order: 3, description: 'Compare NET (network entity title) and IS-type on both sides',
        command: tcmd(p, { nxos: 'show isis', iosxe: 'show isis protocol', eos: 'show isis summary', junos: 'show isis overview' }),
        look_for: 'Area mismatch (NET area-id differs) or level mismatch (L1 vs L2 on opposite sides)' },
      { order: 4, description: 'Check IS-IS authentication configuration',
        command: tcmd(p, { nxos: 'show running-config router isis', iosxe: 'show running-config | section router isis', eos: 'show running-config | section router isis', junos: 'show configuration protocols isis' }),
        look_for: 'Authentication key mismatch — IS-IS hellos silently rejected' },
      { order: 5, description: 'Verify MTU compatibility for IS-IS',
        command: tcmd(p, { nxos: 'show interface <intf> | include MTU', iosxe: 'show interfaces <intf> | include MTU', eos: 'show interfaces <intf> | include MTU', junos: 'show interfaces <intf> | match mtu' }),
        look_for: 'MTU mismatch — IS-IS padding detects this but it can still cause adjacency issues' },
    ],
    causes: [
      { cause: 'IS-IS not enabled on the interface', confidence: 0.8,
        indicators: ['Interface not in "show isis interface"', 'No "isis enable" under interface', 'Wrong instance name'] },
      { cause: 'Area (NET) mismatch between neighbors', confidence: 0.7,
        indicators: ['Different area-id in NET', 'L1 adjacency requires same area', 'show isis shows different NET'] },
      { cause: 'IS-IS level mismatch (L1-only vs L2-only)', confidence: 0.6,
        indicators: ['One side L1-only, other L2-only', 'No common level', 'Adjacency never forms'] },
      { cause: 'Authentication key/type mismatch', confidence: 0.45,
        indicators: ['Auth configured but key differs', 'PDU rejected counter incrementing', 'Hellos sent but no response'] },
    ],
    remediation: [
      'Enable IS-IS on the interface under the correct instance ("isis enable <tag>")',
              ...
              <div>
                ...
                <select
                  ...
                  className="bg-white/5 border border-white/10 rounded px-3 py-2 text-sm text-gray-200
                             focus:outline-none focus:border-blue-500"
                >
                  {Array.from(new Set(TROUBLESHOOT_SYMPTOMS.map(s => s.group))).map(group => (
                    <optgroup key={group} label={group}>
                      {TROUBLESHOOT_SYMPTOMS.filter(s => s.group === group).map(s => (
