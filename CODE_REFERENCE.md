...
## Frontend — `lib/config-facts.ts` (Normalized config facts — AM1/AM4)
...
- `recordAvailability(acc, fleet)` / `availabilityReport(acc)` (T5) — fold per-tick fleet evaluations into a per-device up/total accumulator (`AvailabilityAcc`; up = healthy|degraded, only `down` subtracts) → per-device availability % (worst-first) + fleet mean + sample count. Powers the monitor tab's color-coded "SLA {fleetPct}%" chip (`availRef`).
- `updateAlertHistory(prev, alerts, nowIso)` / `ackAlert(history, key)` / `alertHistoryList(history)` (T6) — session-scoped alert lifecycle (`AlertHistory` keyed by device\|metric: firstSeen→lastSeen, count, clearedAt, acked; re-fire resets). Powers the monitor tab's collapsible "Alert history (session)" with per-alert ack (`alertHistRef`).
- `simulateInterfaces(device, role, tick, aggHint?, portCount=8)` / `analyzeInterfaces(ifaces)` (T7) — deterministic per-interface model (role-aware naming/speed; sticky oper-down; utilization split from device throughput; the device's **aggregate error counters are pinned to one stable culprit port**) + per-port analysis (DOWN critical, CRC 50/200, util 85/95, discards) with up/down summary. Powers the device cards' "▸ ports" drill-down table.
...
- **TAC KB expansion (O1):** `TROUBLESHOOT_SYMPTOMS` expanded to 20 entries (from 8) with `group` field for category grouping; `TROUBLESHOOT_PLAYBOOKS` expanded to 20 full playbooks covering: Routing (bgp_down, ospf_adjacency, routing_loop, isis_adjacency), Overlay (vxlan_evpn), Physical/L2 (interface_flap, stp_loop, mac_flap), Performance (high_latency, packet_loss, mtu_blackhole), Device Health (high_cpu, memory_exhaustion, hardware_failure), Services (dhcp_failure, aaa_auth_failure, ntp_sync), Redundancy (hsrp_vrrp, vpc_mlag), GPU Fabric (pfc_rocev2). UI dropdown uses `<optgroup>` by group.
